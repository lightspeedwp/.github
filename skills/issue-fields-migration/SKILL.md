...
1. If any repo shows `push: false` or `triage: false`, warn the user before proceeding. Items in those repos will fail at write time.
2. Cache the `repository_id` (integer) for each repo now; you will need it in Phase 6:
