...
## Versioning and archive discipline

When a task involves accepted changes, version history, rollback, or repeatable evaluation records, load `references/archive-and-versioning.md`. Use it to decide version bumps, archive fields, and rollback notes.

Use archive records for durable change decisions, not for raw private notes. Keep run-specific observations in local files unless the user explicitly asks to preserve them.

## Evaluation pack

When the user asks to benchmark, compare variants, validate a mutation, or prepare acceptance criteria, load `references/evaluation-pack.md`. Prefer small scenario-based checks over invented numeric benchmarks.

When testing scripts or generated code, run the relevant command in a sandbox and report the command, outcome, and limitations.

## Example scenarios

When trigger or routing behaviour is unclear, load `references/example-scenarios.md` and test against the closest positive, negative, and edge-case examples before recommending a mutation.

## Repeat improvement requests

When the user repeatedly asks to continue improving an already-updated artefact, load `references/improvement-backlog.md` before proposing or applying another batch. Use it to separate high-value ready mutations from speculative polish, and pause when real usage feedback is more valuable than more instructions.

Treat repeated "continue" requests as approval to create another local package only when the next batch is bounded, low-risk, and locally testable. If the next changes are speculative, produce a backlog and recommend testing instead of adding more rules.

## Quality gates

Before packaging an updated skill or reusable workflow, load `references/quality-gates.md`. Run the local quality script when available, then run the standard skill packager. Report validation results, skipped checks, accidental-file cleanup, and rollback path.
...
Use `scripts/validate_evolution_archive.py` when the user provides JSONL archive records and wants a quick schema check before review, handoff, or packaging.

Use `scripts/validate_mutation_proposals.py` when the user provides machine-readable proposal records or wants mutation proposals checked before review, approval, archive, or packaging. Load `references/mutation-proposal-schema.md` for the required fields and review rules.

Use `scripts/check_skill_quality.py` before packaging an updated skill when the user wants a quality gate, repeated improvement batch, or handoff-ready package.

Examples:

```bash
python scripts/evolution_log.py --skill self-evolving-agent --source feedback.txt --output evolution-observations.jsonl
python scripts/validate_evolution_archive.py --input evolution-archive.jsonl
...
- `references/archive-and-versioning.md`: version bump rules, archive fields, rollback discipline, and record examples.
- `references/evaluation-pack.md`: scenario tests, regression checklist, and variant comparison guidance.
- `references/example-scenarios.md`: positive, negative, and edge-case examples for routing and trigger checks.
- `references/improvement-backlog.md`: backlog statuses, pause conditions, and batch selection checks for repeated improvement requests.
- `references/mutation-proposal-schema.md`: machine-readable proposal fields, review rules, and JSON examples.
