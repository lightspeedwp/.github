...
def audit_headings(root: etree._Element, part: str) -> list[Finding]:
    ...
    for p in root.xpath(".//w:p", namespaces=NS):
        ...
        lvl = _heading_level_from_style(pstyle.get(f"{{{W_NS}}}val") if pstyle is not None else None)
        if lvl is None:
            continue
        ...
        if (descr.strip() == "") and (title.strip() == ""):
            findings.append(
                Finding(
                    ...
                    context={"part": part, "id": docpr.get("id"), "name": docpr.get("name")},
                )
            )
    return findings
...
def apply_fixes(in_docx: str, out_docx: str, fix_image_alt: str | None, fix_table_headers: str | None) -> dict[str, int]:
...
def main() -> None:
    ...
    report = audit_docx(args.out if (args.out and (args.fix_image_alt or args.fix_table_headers)) else args.in_docx)
