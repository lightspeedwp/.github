...
NS = {"w": W_NS, "r": "http://schemas.openxmlformats.org/officeDocument/2006/relationships"}
...
def add_page_number_field_to_footer(docx_path: Path) -> None:
    ...
    r = p.add_run()
    fld_begin = OxmlElement("w:fldChar"); fld_begin.set(docx_qn("w:fldCharType"), "begin")
    instr = OxmlElement("w:instrText"); instr.set(docx_qn("xml:space"), "preserve"); instr.text = " PAGE \\* MERGEFORMAT "
    fld_sep = OxmlElement("w:fldChar"); fld_sep.set(docx_qn("w:fldCharType"), "separate")
    txt = OxmlElement("w:t"); txt.text = "1"
    fld_end = OxmlElement("w:fldChar"); fld_end.set(docx_qn("w:fldCharType"), "end")
...
def hyperlink_first_paragraph(docx_path: Path, url: str, underline: bool = True) -> None:
    ...
    color = OxmlElement("w:color"); color.set(docx_qn("w:val"), "0000FF"); rPr.append(color)
    if underline:
        u = OxmlElement("w:u"); u.set(docx_qn("w:val"), "single"); rPr.append(u)
    r.append(rPr)
...
def main() -> int:
    ...
    # OOXML switches
    ap.add_argument("--enable-track", action="store_true", help="Enable <w:trackRevisions/> in settings.xml")
    ...
    ap.add_argument("--tracked-replace-ins-id", default=None, help="Convert <w:ins w:id=...> into delete+insert")
    ...
    ap.add_argument("--add-comment", action="store_true", help="Add a Word comment via comments.xml")
    ap.add_argument("--comment-text", default="", help="Comment body")
    ap.add_argument("--indent-left-twips", type=int, default=None, help="Match paragraphs by indentation left (twips)")
    ...
    # python-docx helpers
    ap.add_argument("--header-date", default=None, help="Set first section header to this text (right aligned)")
    ap.add_argument("--add-page-numbers", action="store_true", help="Add a centered page number field to footer")
    ap.add_argument("--hyperlink-first", default=None, help="Add hyperlink to first paragraph text to this URL")
