...
def _replace_in_text_node(t_node: etree._Element, old: str, new: str, cid_start: int, author: str, when: str) -> tuple[int, bool]:
    txt = t_node.text or ""
    if old not in txt:
...
def add_tracked_replacements(in_docx: str, out_docx: str, replaces: list[tuple[str, str]], author: str) -> None:
    ...
    with zipfile.ZipFile(in_docx, "r") as zin:
        ...
        settings_root = etree.fromstring(zin.read(settings_name)) if settings_name in zin.namelist() else etree.Element(w("settings"))
