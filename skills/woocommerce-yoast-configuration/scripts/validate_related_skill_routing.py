#!/usr/bin/env python3
"""Validate related-skill routing between woocommerce-yoast-configuration and woocommerce-yoast-auditor."""
...
REQUIRED_TOKENS = [
    ...
    "route implementation to `woocommerce-yoast-auditor`",
]
...
def main(root='.'):
    ...
    forbidden_standalone = ["`yoast-" + "auditor`", "skills://yoast-" + "auditor", "`yoast-" + "configuration`", "skills://yoast-" + "configuration"]
    forbidden_hits = [token for token in forbidden_standalone if token in corpus]
    if forbidden_hits:
        print({"forbidden_generic_skill_routes": forbidden_hits})
        return 1
