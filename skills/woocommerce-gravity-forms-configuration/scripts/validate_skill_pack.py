#!/usr/bin/env python3
"""Local structural validator for the WooCommerce Gravity Forms Configuration skill package.
...
def validate_frontmatter(root: Path) -> int:
    ...
    fm = skill[4:end]
    if "name: woocommerce-gravity-forms-configuration" not in fm:
        return fail("SKILL.md name must be woocommerce-gravity-forms-configuration")
...
def validate_test_prompt_count(root: Path) -> int:
    ...
    if count < 135:
        return fail(f"expected at least 135 numbered test prompts, found {count}")
    return 0
...
def validate_skill_scope_names(root: Path) -> int:
    status = 0
    skill = read(root / "SKILL.md")
    if "name: woocommerce-gravity-forms-configuration" not in skill:
        status |= fail("SKILL.md name must be woocommerce-gravity-forms-configuration")
    banned_patterns = [
        (re.compile(r"(?<!woocommerce-)gravity-forms-auditor"), "generic gravity-forms-auditor reference"),
        (re.compile(r"(?<!woocommerce-)gravity-forms-configuration"), "generic gravity-forms-configuration reference"),
        (re.compile(r"woocommerce-woocommerce", re.I), "double WooCommerce naming"),
        (re.compile(r"tour\s*operator|tour-operator|tour_operator", re.I), "tour operator reference"),
    ]
    for path in root.rglob("*"):
        ...
        for pattern, label in banned_patterns:
            if pattern.search(text):
                status |= fail(f"{label} found in {path.relative_to(root)}")
    if (root / "profiles/wordpress-configuration-agent.md").exists():
        status |= fail("generic WordPress profile should not be bundled in this WooCommerce-specific skill")
    return status
...
def main(argv: list[str]) -> int:
    ...
    status |= validate_skill_scope_names(root)
    if status == 0:
