#!/usr/bin/env python3
"""Local structural validator for the Tour Operator Gravity Forms Configuration skill package.
...
def validate_frontmatter(root: Path) -> int:
    ...
    fm = skill[4:end]
    if "name: tour-operator-gravity-forms-configuration" not in fm:
        return fail("SKILL.md name must be tour-operator-gravity-forms-configuration")
