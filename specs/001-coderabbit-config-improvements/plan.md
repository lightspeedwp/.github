...
Restructure and expand the `.coderabbit.yml` configuration file to provide comprehensive, branch-type-specific review instructions for all 30+ branch types and 95%+ of file types in the repository. Implementation follows explicit path pattern priority/specificity ordering (specific patterns override general patterns), with external audit guide for coverage verification.

**Key Outcomes**:
- All 30+ branch types get customized review context (feat/, fix/, security/, perf/, etc.)
...
- Zero breaking changes to existing CodeRabbit workflows

## Technical Context

**Project Type**: Configuration/Tooling (YAML configuration file for CodeRabbit)

**Primary Asset**: `.coderabbit.yml` (443 lines → expanded to ~800-1000 lines with all improvements)

**Storage**: YAML format (in version control at `.github/coderabbit.yml`)

**Testing**: 
- Manual review of config structure against specification
- CodeRabbit review quality audit (examine sample PRs to verify branch-type context is applied)
- Coverage audit using external guide

**Target Platform**: GitHub (CodeRabbit integration at organization level)
...
**Constraints**: 
- MUST maintain backward compatibility (no breaking changes to existing path_instructions)
...
**Scale/Scope**:
- 30+ branch types requiring context-aware review instructions
- 95%+ file type coverage (baseline: ~50 current file types/paths)
- ~15-20 new instruction blocks to add
- ~30-40 existing instruction blocks to enhance
...
**Estimated**: 12-15 implementation tasks covering:
- Config audit & analysis
- Branch-type-specific sections
- Path pattern reorganization
- Instruction enhancement
- Audit guide creation
- Testing & validation
