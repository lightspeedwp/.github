...
async function run() {
  try {
    ...
    const repoOwner = (event.repository && event.repository.owner.login) || orgLogin;
    ...
    let statusValue = null, priorityValue = null, typeValue = null;
    if (isPR) {
    ...
    const labels = (isPR ? event.pull_request.labels : event.issue.labels) || [];
    ...
    console.log(`Determined field values -> Status: ${statusValue || 'None'}, Priority: ${priorityValue || 'None'}, Type: ${typeValue || 'None'}`);
    ...
    const updateField = async (fieldName, fieldValue) => {
      ...
      const field = findField(fieldName);
      if (!field) {
        console.warn(`Field '${fieldName}' not found on project, skipping update.`);
        return;
      }
      let fvInputs = {};
      if (field.dataType === "SINGLE_SELECT") {
        ...
        if (!optId) {
          console.warn(`Option '${fieldValue}' not found for field '${fieldName}', skipping update.`);
          return;
        }
      ...
      `, { project: projectId, itemId: itemNodeId, fieldId: field.id, fv: fvInputs });
    ...
    if (statusValue) {
      ...
      const statusLabelsToRemove = labels
        .filter(l => l.name.startsWith("status:") && !l.name.includes(statusValue.toLowerCase().replace(/ /g, "")));
      ...
      if (statusLabelsToRemove.length) {
        console.log(`Removed obsolete status labels: ${statusLabelsToRemove.map(l=>l.name).join(", ")}`);
      }
    }
