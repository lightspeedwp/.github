...
- A target label already exists in a repository (for example `status:done` next to `status:completed`), so an in-place rename is impossible: the source label is replaced on every issue and PR first, then retired. This is decided per repository: where the target does not exist, the label is renamed in place (FR-012)
...
- **FR-016**: After the FR-012 import is approved, the approved label set is `labels.yml` on `develop` at the commit recorded in each repository's dry run (`approved_set_commit`); if `develop` changes that file afterwards, a new dry run is needed. Every label outside it MUST be deleted from all `lightspeedwp` repositories, because unapproved GitHub labels sync into Linear. There are no repository-specific exceptions (closed issue #95 allowed them; this replaces it); excluding archived repositories and forks is a scope rule, not an exception. Deletion MUST be gated: a new gate issue replaces closed issue #95 in `label-governance-policy.yml`, `destructive_cleanup.enabled` stays `false` in the repository (the policy file records the gate issue and approved labels, not an on/off switch), the deletion tooling deletes only when run with an explicit apply flag naming the gate issue and only for repositories whose dry run is marked approved, refusing otherwise, and a per-repository dry-run list is approved by @ashley before anything is deleted. Approval is a comment on the gate issue by @ashleyshaw reading `Approved: <repo> dry run <generated_at>`; the tool checks that the author, repository and timestamp match the dry-run file, so an approval cannot be reused for a newer dry run. Labels still applied to open issues or PRs MUST be migrated to their approved equivalent first; a label that has a mapping target MUST also be migrated on closed issues and PRs, while an unmapped label is deleted from closed items and recorded in the snapshot. GitHub Discussions are treated the same as issues: dry runs and snapshots count them, and open Discussions are migrated first. Deletion covers every non-archived `lightspeedwp` repository where the FR-018 GitHub App is installed, template repositories included; archived repositories and forks are excluded and listed in the gate issue, and a repository without the App blocks its own deletion until the App is installed or @ashley excludes it on the gate issue. Each repository's dry-run summary is posted as a comment on the gate issue mentioning the team that owns the repository; deletion may run as soon as @ashley approves it. Before anything is deleted, a per-repository snapshot (each label's name, colour and description, and the numbers of the issues, PRs and Discussions carrying it) MUST be saved to `evidence/dry-run/{repo}.json`; a deletion is rolled back by recreating the label from the snapshot and reapplying it to the recorded items, and a rename by renaming back. The inventory and deletion tooling MUST paginate so that no label is missed (`label-sync.js` reads only the first 100)
- **FR-017**: To prevent drift after consolidation, a scheduled workflow MUST run at least weekly, compare every `lightspeedwp` repository's labels and the Linear workspace labels against `labels.yml`, and open or update a single report issue listing every difference. Automation that creates labels MUST create only labels defined in `labels.yml`. Label creation from Linear's GitHub integration MUST be restricted before Stage 3 starts (not left until Linear clean-up), so the sync cannot add labels while GitHub labels are changed and deleted; repository label creation by people SHOULD be restricted where the platform allows, without reducing anyone's existing repository access; the drift check and the automation rule are the required controls
- **FR-018**: Organisation-wide label changes and the drift check MUST authenticate with a GitHub App installed across the `lightspeedwp` organisation with only Issues (read/write) and Metadata (read) permissions, using short-lived installation tokens (the repository already stores `BOT_PR_APP_*` secrets for an App). The drift check reads Linear with a read-only API key stored as the `LINEAR_API_KEY` repository secret. Deletion runs (FR-016) and Linear write operations are performed from @ashley's own session, not from CI, using a fine-grained personal access token limited to `lightspeedwp` repositories with Issues and Discussions (read/write) and Metadata (read) permissions, expiring in 7 days or less, created for the run and revoked afterwards. No credential may be committed to the repository
- **FR-019**: `.github/issue-types.yml` (after the FR-014 swap) is the source of truth for issue types. The GitHub organisation's native issue types MUST be changed to match it exactly, one native type per entry, in this order because the organisation is limited to 25 types: (1) rename A11y → Accessibility, Code Refactor → Refactor, Code Review → Review, Build & CI → CI; (2) migrate every issue, open and closed, using Maintenance, Story or Integration to its mapped type (Maintenance → Chore plus `area:maintenance`, Integration → Feature plus `area:integration`, Story → Feature), updating its `type:*` label to match, then remove those three types; (3) add Build, Dependency Update and Decision. There is no native Question type, so FR-014's organisation step is covered by adding Decision here. These steps run only after the `[ISSUE-TYPE-UPDATE-REQUEST]` #3556 is approved
...
- **FR-023**: Consolidation runs MUST be safe to stop, repeat and undo. (1) Each repository records `executed_at` in its dry-run file when its run finishes; a re-run skips finished repositories and continues with the rest, and rollback happens only when @ashley asks for it on the gate issue. (2) Every stage is idempotent: the tooling compares the current state with the approved set and makes no write when nothing differs. (3) A rejected dry run skips that repository; a corrected dry run needs a new approval, and any change to the dry run or to `labels.yml` invalidates the earlier approval (FR-016). (4) Before deleting, the tooling re-reads the repository; if its labels, or the items carrying a listed label, differ from the approved dry run, it skips that repository and asks for a new dry run and approval. (5) Every Linear write is logged (issue, old label, new label, time) to `evidence/linear-writes.json`; a Linear merge is rolled back by reapplying the source label from the log and restoring the retired label. (6) Mutating requests run one at a time, at least one second apart, and honour `Retry-After` and `x-ratelimit-reset`; Linear calls respect Linear's complexity limits; when a limit is hit the run pauses and then resumes as in (1). (7) Linear labels are identified by ID and scope (workspace or team), never by name alone, so a team label that shares a workspace label's name is mapped separately. (8) Labels that differ only by case or spacing (for example `scope: website`) are separate sources, each with its own mapping entry, and are never merged automatically; because GitHub label names ignore case, a case-only change is an in-place rename. (9) If a FR-019 step fails (for example the API is unavailable), the run stops, records the step reached in `evidence/native-issue-types.json` and resumes from that step; FR-019's order frees type slots before new types are added, so the 25-type limit is never exceeded. (10) Every destructive run appends one record per change (who ran it, when, repository, action, label, before and after) to `evidence/consolidation-log.json` and posts a summary comment on the gate issue
...
- Renaming a GitHub label in place keeps it on every issue, PR and Discussion that carries it (research R1). Before Stage 3, this is confirmed by renaming a test label on a test issue in a sandbox repository and checking that the label is still applied
...
- Q: When should `labels.yml`, the `pr_aiops.md` PR template, and the scripts, workflows and tests that use the old names switch to `aiops` and `spec`? → A: In a separate configuration PR, approved through new `[LABEL-UPDATE-REQUEST]` and `[TEMPLATE-UPDATE-REQUEST]` issues, merged only after #3564 and together with the GitHub label renames; #3362 stays audit-only
- Q: About 20 `openspec:` label names used in scripts and docs are not in `labels.yml`. What should the rename do with them? → A: Rename the prefix everywhere; record each one as a gap so the T043 mapping maps it to one of the 9 `spec:*` labels or retires it
- Q: About 170 references use labels like `spec:001` to mark which spec an issue belongs to. Should the spec-number labels move to a different prefix? → A: Yes. They move to `spec-id:NNN` (for example `spec-id:001`) in the configuration PR, updating the plan, the bulk-labelling script and its test
- Q: When a label is deleted, what happens to closed issues and PRs that carry it? → A: Labels with a mapping target are migrated on open and closed items; unmapped labels are deleted from closed items, and the snapshot records which items had them
- Q: Should labels on GitHub Discussions be treated like labels on issues and PRs? → A: Yes. Dry runs and snapshots count Discussions, and open Discussions are migrated before deletion
- Q: Which repositories are in scope for label deletion? → A: Every non-archived repository where the GitHub App is installed, templates included; archived repositories and forks are excluded and listed in the gate issue; a repository without the App blocks until the App is installed or @ashley excludes it
...
- Q: If a deletion run stops part-way, what should happen? → A: Resume: finished repositories record `executed_at` and are skipped on re-run; rollback only when @ashley asks on the gate issue (FR-023)
- Q: If someone creates or applies a label in a repository after its dry run was approved, what should the deletion run do? → A: Re-read the repository before deleting; if it differs from the dry run, skip it and ask for a new dry run and approval (FR-023)
- Q: How should the Linear↔GitHub sync be stopped from recreating labels while Stages 3–4 run? → A: Restrict label creation in Linear's GitHub integration before Stage 3 starts; the FR-023 re-read before deletion catches anything that still slips through (FR-017)
- Q: Which credential should @ashley's session use for deletion runs and Linear writes? → A: A fine-grained personal access token limited to `lightspeedwp` repositories (Issues and Discussions read/write, Metadata read), expiring in 7 days or less and revoked after the run (FR-018)
