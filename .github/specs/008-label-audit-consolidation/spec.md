...
- A label change request contradicts an approved merge or rename (for example #3554 asks for `area:observability` as its own label while FR-012 merges it into `area:monitoring`): the conflict is resolved in this specification before the request is approved, so the mapping never holds both (FR-012)
