...
- Q: When a session on this repository uses the GitHub tools to write to another LightSpeed repository, which of the guard's rules apply there? → A: Only the organisation-wide naming rules (forbidden prefixes, the placeholder and `{type}/{scope}-{title}`, for branch creation, file writes and PR heads). The protected-branch rules, the documentation exception and the `main` base rule apply only to this repository, whose base branch the guard knows (FR-008, FR-009).
- Q: SC-005 says session start adds no more than 30 seconds when dependencies are current. How should it be verified? → A: By an automated session-start contract test that runs on every PR through the FR-023 workflow. The first-run 5-minute target stays a manual check during the quickstart run.
...
- **Other LightSpeed repositories**: Only branch names are checked there (FR-009 scope). Writes to their `main` or `develop` aren't refused by this guard, because their base branch may differ; their own branch protection applies.
...
- **FR-009**: Opening a PR on a LightSpeed repository (one whose GitHub owner is `lightspeedwp`, compared case-insensitively), through the GitHub MCP tools, `gh pr create` or `gh api`, MUST be refused when the head branch is non-compliant. On this repository, it MUST also be refused when the base is `main` and the head is not a `release/*` or `hotfix/*` branch.
  ...
  **Scope across repositories**: on other LightSpeed repositories, the guard applies only the naming rules: it refuses creating a branch, writing files to a branch, or opening a PR from a head whose name is non-compliant or the placeholder (FR-007, and FR-008 and FR-009 for names). The protected-branch rules, the documentation exception and the `main` base rule apply only to this repository.
...
- **SC-005**: Session start adds no more than 30 seconds when dependencies are already current. The first provisioning run completes in under 5 minutes. The 30-second target is checked by an automated contract test on every PR (FR-023); the 5-minute first run is checked manually during the quickstart run.
