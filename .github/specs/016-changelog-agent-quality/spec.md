...
**Note (2026-09-22)**: Renumbered from `015` to `016` — `015` is used on `develop` by `015-pr-agent-consolidation` (#3403), and was also claimed by `015-ci-failure-remediation` on #3367, since removed. The validation engine referenced in User Story 1 has since shipped to `develop` (`.github/validation/changelog/`, wired into `.github/workflows/changelog-unified.yml`); it already provides local execution (`npm run validate:changelog` from within that package, or `.github/validation/changelog/validate.sh`), 8 documented rules, and JSON/text output. User Story 1 has been rescoped below to close the remaining gap — a convenience alias at the repo root — rather than rebuild what already exists. User Stories 2–4 remain open gaps as originally scoped.
...
- **FR-008**: Validation workflow MUST run on every PR that modifies CHANGELOG.md and provide feedback via GitHub PR comments or status checks. FR-009's bypass takes precedence where the two overlap: see the changelog-only case noted there
- **FR-009**: Workflow MUST block merge if changelog entries fail validation, with the same bypasses as the shipped gate: Dependabot and docs-bot authors, docs-only diffs (every changed file under `docs/**` or ending in `.md`), and the `meta:no-changelog` label. Branch-name prefix is deliberately not a bypass, so a `chore/` branch with a code diff still needs a changelog entry or the label. **Precedence over FR-008**: the shipped docs-only test is `file.startsWith('docs/') || file.endsWith('.md')`, and `CHANGELOG.md` ends in `.md`, so a pull request that changes *only* `CHANGELOG.md` satisfies the docs-only test and is therefore exempt from the changelog requirement. That is a defect in the shipped gate, not intended behaviour: a changelog-only pull request is exactly the one that most needs its entries validated. FR-009 keeps the shipped behaviour for compatibility and FR-008 records the intent; closing the gap means excluding `CHANGELOG.md` from the docs-only exemption
