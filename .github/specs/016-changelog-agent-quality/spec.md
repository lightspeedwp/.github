...
- The changelog requirement follows the shipped gate in `.github/workflows/changelog-unified.yml`, whose tests run in this order: Dependabot and docs-bot authors are skipped; a pull request containing both `meta:needs-changelog` and `meta:no-changelog` fails, as does `meta:no-changelog` on a high-impact release-related change type; a diff containing the root `CHANGELOG.md` is validated; then docs-only diffs (`docs/**` or `*.md`) are skipped, as is `meta:no-changelog`; any other pull request fails, needing a `CHANGELOG.md` update or the label
