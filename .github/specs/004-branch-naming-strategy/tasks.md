...
- 38 authorised branch types (from Constitution Section VIII) with pattern `{type}/{scope}-{title}`
...
- [ ] T002 Create branch type definitions at `.github/branch-validation/branch-types.json` (38 authorised types from Constitution Section VIII)
...
**Goal**: System validates branch names match pattern `{type}/{scope}-{title}` with 38 authorised types (Constitution Section VIII), 0 forbidden prefixes, with documented branch exemptions
...
- [ ] T014 [P] [US1] Unit test for type validation at `.github/branch-validation/test/unit/test-type-validation.js` (accept all 38 authorised types, reject others)
...
- **Phase 3 (US1 - FR-1)**: 11 tasks (5 tests, 6 implementation)
- **Phase 4 (US2 - FR-2)**: 8 tasks (3 tests, 5 workflows/tools)
- **Phase 5 (US3 - FR-3)**: 7 tasks (3 tests, 4 workflow)
- **Phase 6 (US4 - FR-4)**: 8 tasks (3 tests, 5 tools/docs)
- **Phase 7 (US5 - FR-5)**: 9 tasks (3 tests, 6 implementation)
- **Phase 8 (Polish)**: 8 tasks

**TOTAL: 64 tasks**

**Parallelizable**: 39 tasks marked [P]  
**MVP Scope**: Phases 1-2-3 = 24 tasks (1.5 weeks for 1 FTE)  
