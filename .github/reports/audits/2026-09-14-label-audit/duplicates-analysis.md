...
- **2 overlaps are already approved** in spec 008 FR-012: `status:completed` → `status:done` and `area:tests` → `area:testing`.
- **8 proposed consolidations need @ashley's decision** before they go into the Stage 1 `[LABEL-UPDATE-REQUEST]`: the two priority scales, `status:needs-qa` / `status:needs-testing`, `area:block-editor` / `comp:block-editor`, three `area:*` labels that repeat `ai-ops:*` labels, and `area:compatibility`.
...
| `area:agents`        | `ai-ops:agents`          | high       | merge  | `aiops:agents`         | needs decision | After the FR-011 rename, keep aiops:agents for AI assets and retire area:agents.                                                                                  |
| `area:instructions`  | `ai-ops:instructions`    | high       | merge  | `aiops:instructions`   | needs decision | As above: keep aiops:instructions, retire area:instructions.                                                                                                      |
| `area:prompts`       | `ai-ops:prompts`         | high       | merge  | `aiops:prompts`        | needs decision | As above: keep aiops:prompts, retire area:prompts.                                                                                                                |
...
| `area:ai`            | `area:agents`          | low        | keep   | `both` | keep   | Not duplicates: area:ai is the umbrella for area:agents, area:skills, area:instructions and area:prompts; they are nested rather than duplicate.                |
