...
const { MilestoneAssignmentAgent } = require('../agents/includes/milestone-assignment.js');
...
async function main() {
  try {
    ...
    core.info(`Starting milestone assignment for ${issues.length} issues (dry-run: ${dryRun})`);
    ...
    // Summary stats
    const assigned = results.filter((r) => r.status === 'assigned' || r.status === 'dry-run-success').length;
    const errors = results.filter((r) => r.status === 'error').length;
    const skipped = results.filter((r) => r.status === 'no-milestone-found').length;

    core.info(`✅ Milestone assignment complete: ${assigned} assigned, ${skipped} skipped, ${errors} errors`);
    process.exit(0);
  } catch (error) {
