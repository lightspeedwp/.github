...
function checkTriggerTypes() {
  ...
  for (const filename in reusableFiles) {
    ...
    if (typeof triggers === 'object' && 'workflow_call' in triggers) {
      ...
      const triggerKeys = typeof triggers === 'object' ? Object.keys(triggers) : [triggers];
      warnings.push(
  ...
  for (const filename in controlPlaneFiles) {
    ...
    } else if (typeof triggers === 'object' && Object.keys(triggers).length === 1 && 'workflow_call' in triggers) {
      warnings.push(
