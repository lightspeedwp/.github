...
### 6.2 Example CI Check (Pseudocode)
      ...
      - name: Check projects have issue links
        run: |
          for project in .github/projects/active/*/; do
            if ! grep -q "Related Issues" "$project/README.md"; then
              echo "❌ $project missing Related Issues section"
              exit 1
            fi
          done
      - name: Validate issue numbers are real
        run: |
          # grep for #XXXX and validate against GitHub API
```
...
- [PLANNING.md](./PLANNING.md) — Project restructuring initiative planning
- [AUDIT_RESULTS.md](./AUDIT_RESULTS.md) — Current linking gaps & recommendations
