 ...
 * @version 1.0.0
 * @see .github/agents/agent-name.agent.md
 */
...
jobs:
  release:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test
      - run: node .github/agents/release.agent.js --scope=${{ inputs.scope }}
...
jobs:
  metrics:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: node .github/agents/metrics.agent.js
...
jobs:
  sync:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: node .github/agents/project-meta-sync.agent.js
        env:
...
jobs:
  review:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: node .github/agents/reviewer.agent.js
```
...
jobs:
  plan:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: node .github/agents/planner.agent.js
```
