...
tags: ["accessibility", "a11y", "wcag", "wordpress"]
license: "GPL-3.0"
---
...
In addition to producing accessible code, GitHub Copilot and similar tools must also demonstrate respectful and bias-aware behaviour in accessibility contexts. All generated output must follow these principles:
...
- Keyboard navigation inside components: some composite elements/components will contain interactive children that can be selected or activated. Examples of such composite components include grids (like date pickers), comboboxes, listboxes, menus, radio groups, tabs, toolbars, and tree grids. For such components:
  ...
  - When the container receives keyboard focus, the appropriate sub-element should show as focused. This behaviour depends on context. For example:
...
- The contrast of text against the background colour must be at least 4.5:1. Large text, must be at least 3:1. All text must have sufficient contrast against it's background colour.
  - Large text is defined as 18.5px and bold, or 24px.
  - If a background colour is not set or is fully transparent, then the contrast ratio is calculated against the background colour of the parent element.
- Parts of graphics required to understand the graphic must have at least a 3:1 contrast with adjacent colours.
- Parts of controls needed to identify the type of control must have at least a 3:1 contrast with adjacent colours.
- Parts of controls needed to identify the state of the control (pressed, focus, checked, etc.) must have at least a 3:1 contrast with adjacent colours.
- Colour must not be used as the only way to convey information. E.g., a red border to convey an error state, colour coding information, etc. Use text and/or shapes in addition to colour to convey information.
...
- Interactive elements must have appropriate roles and keyboard behaviours.
