#!/usr/bin/env bats
# Test: status-enforcer.bats
# Description: Bats tests for status-enforcer.js utility script
...
setup() {
  ...
  SCRIPT="$(cd "$(dirname "$BATS_TEST_FILENAME")/../../scripts/utility" && pwd)/status-enforcer.js"
  chmod +x "$SCRIPT"
}
