...
const GUARDRAILS = {
  ...
  performance: {
    ...
    rules: {
      ...
      concurrencyControl: {
        enabled: true,
        message: 'Workflow should define concurrency to prevent overlapping runs',
        level: 'warning',
      },
      // Cache dependencies
      cacheDependencies: {
  ...
  quality: {
    ...
    rules: {
      ...
      errorHandling: {
        enabled: true,
        message: 'Use continue-on-error carefully; most jobs should fail on error',
        level: 'warning',
      },
    },
  },
  ...
  consistency: {
    ...
    rules: {
      ...
      osVersion: {
        enabled: true,
        message: 'Use ubuntu-latest for consistency (avoid ubuntu-20.04, ubuntu-22.04)',
        level: 'warning',
      },
    },
  },
...
class WorkflowValidator {
  ...
  validateSecurity(filename, workflow) {
    ...
    if (this.guardrails.security.rules.explicitPermissions.enabled) {
      if (!workflow.permissions && Object.keys(workflow.jobs || {}).length > 0) {
        this.addWarning(filename, 'Missing permissions block (should default to contents: read)');
      }
    }
      ...
      for (const [jobName, job] of Object.entries(workflow.jobs || {})) {
        for (const step of job.steps || []) {
          if (step.run) {
            ...
            if (step.run.includes('${{ secrets.') || step.run.includes('${{ secrets[')) {
              this.addError(
                filename,
  ...
  validatePerformance(filename, workflow) {
    ...
    if (this.guardrails.performance.rules.concurrencyControl.enabled) {
      if (!workflow.concurrency) {
        this.addWarning(filename, 'Consider adding concurrency to prevent overlapping runs');
        hasWarnings = true;
      }
    }
    ...
    if (this.guardrails.performance.rules.cacheDependencies.enabled) {
      ...
      if (hasNpmInstall && !hasCache) {
        this.addWarning(filename, 'Consider adding caching for dependencies (npm/pip)');
        hasWarnings = true;
      }
    }
    ...
    if (this.guardrails.performance.rules.checkoutDepth.enabled) {
      const jobsContent = JSON.stringify(workflow.jobs || {});
      if (jobsContent.includes('actions/checkout') && !jobsContent.includes('fetch-depth')) {
        this.addWarning(filename, 'Consider using fetch-depth for faster checkout');
        hasWarnings = true;
      }
    }
  ...
  validateQuality(filename, workflow) {
    ...
    if (this.guardrails.quality.rules.clearJobNames.enabled) {
      for (const jobName of Object.keys(workflow.jobs || {})) {
        if (jobName.length < 3) {
          this.addWarning(filename, `Job name "${jobName}" is too short; use descriptive names`);
          hasWarnings = true;
        }
      }
    }
    ...
    if (this.guardrails.quality.rules.nodeVersion.enabled) {
      ...
      if (jobsContent.includes('setup-node') && !jobsContent.includes('20')) {
        ...
        if (nodeMatch && nodeMatch[1] < 18) {
          this.addWarning(filename, `Node version ${nodeMatch[1]} is outdated; use >=18`);
          hasWarnings = true;
        }
      }
    }
    ...
    if (this.guardrails.consistency.rules.osVersion.enabled) {
      for (const [jobName, job] of Object.entries(workflow.jobs || {})) {
        if (job['runs-on'] && job['runs-on'] !== 'ubuntu-latest') {
          if (!job['runs-on'].includes('macos') && !job['runs-on'].includes('windows')) {
            this.addWarning(
              filename,
