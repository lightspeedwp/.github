...
function main() {
  ...
  const registryPath = path.join(root, 'workflows', 'memory', 'registry', 'memory-registry.yaml');
  const lockPath = path.join(root, 'workflows', 'memory', 'registry', 'inventory-lock.json');
  const profilesAgentDir = path.join(root, 'workflows', 'memory', 'profiles', 'agents');
  const profilesSkillDir = path.join(root, 'workflows', 'memory', 'profiles', 'skills');
  const examplesAgentDir = path.join(root, 'workflows', 'memory', 'examples', 'agents');
  const examplesSkillDir = path.join(root, 'workflows', 'memory', 'examples', 'skills');
  ...
  const registrySchema = loadJson(path.join(schemaDir, 'memory-registry.schema.json'));
  const profileSchema = loadJson(path.join(schemaDir, 'memory-profile.schema.json'));
  const recordSchema = loadJson(path.join(schemaDir, 'memory-record.schema.json'));
  const snapshotSchema = loadJson(path.join(schemaDir, 'memory-snapshot.schema.json'));
  const packSchema = loadJson(path.join(schemaDir, 'memory-example-pack.schema.json'));
  ...
  const profileFiles = [
    ...listFilesRecursive(profilesAgentDir, (full) => full.endsWith('.memory-profile.yaml')),
    ...listFilesRecursive(profilesSkillDir, (full) => full.endsWith('.memory-profile.yaml')),
  ].sort();

  const exampleFiles = [
    ...listFilesRecursive(examplesAgentDir, (full) => full.endsWith('.memory.example.yaml')),
    ...listFilesRecursive(examplesSkillDir, (full) => full.endsWith('.memory.example.yaml')),
  ].sort();
  ...
  if (!config.profilesOnly) {
    for (const exampleFile of exampleFiles) {
      ...
      if (!fs.existsSync(profileRefPath)) {
        errors.push(`Example profile_ref missing (${relPath}): ${data.profile_ref}`);
      }

      for (const record of data.records) {
        if (record.asset_id !== data.asset_id) {
          errors.push(`Record asset_id mismatch in ${relPath}: ${record.record_id}`);
        }
        if (record.asset_type !== data.asset_type) {
          errors.push(`Record asset_type mismatch in ${relPath}: ${record.record_id}`);
        }
      }
    }
  ...
  if (!config.examplesOnly) {
    ...
    for (const profileSource of agentProfileSources) {
      if (!agentFiles.includes(profileSource)) {
        errors.push(`Agent profile references non-existent source: ${profileSource}`);
      }
    }

    for (const profileSource of skillProfileSources) {
      if (!skillFiles.includes(profileSource)) {
        errors.push(`Skill profile references non-existent source: ${profileSource}`);
      }
    }
    ...
    if (fs.existsSync(lockPath)) {
      ...
      for (const requiredAgent of requiredAgents) {
        if (!agentFiles.includes(requiredAgent)) {
          errors.push(`Inventory lock required agent path missing: ${requiredAgent}`);
        }
      }
      for (const requiredSkill of requiredSkills) {
        if (!skillFiles.includes(requiredSkill)) {
          errors.push(`Inventory lock required skill path missing: ${requiredSkill}`);
        }
      }
    } else {
