...
Also [third](../../c.md).`
    ...
    it('should extract links with anchors', () => {
      const content = 'Jump to [section](#heading) or [file](./doc.md#section).'
      const links = extractLinks(content)
    ...
    it('should not extract non-markdown links', () => {
      const content = 'Regular text (not a link) and <a href="#">HTML link</a>.'
      const links = extractLinks(content)
  ...
  describe('validateLinks', () => {
    it('should pass when all links exist', () => {
      ...
      const errors = validateLinks(content, '/root/index.md', mockExists(files))
    ...
    it('should skip absolute URLs', () => {
      const files = new Set(['/root/file.md'])
      const content = 'Visit [Google](https://google.com) and see [file](./file.md).'
      const errors = validateLinks(content, '/root/index.md', mockExists(files))
...
Also [file3](./file3.md).`
    ...
    it('should resolve parent directory links', () => {
      ...
      const errors = validateLinks(content, '/root/docs/index.md', mockExists(files))
    ...
    it('should handle nested directory structures', () => {
      ...
      const errors = validateLinks(content, '/root/api/docs/index.md', mockExists(files))
...
[missing](./missing.md)`
    ...
    it('should handle very long paths', () => {
      const longPath = './very/deep/nested/directory/structure/with/many/levels/file.md'
