...
function buildTempRepo() {
  ...
  mkdirp(path.join(root, 'workflows', 'memory', 'examples', 'packs', 'minimal'));
  mkdirp(path.join(root, 'workflows', 'memory', 'registry'));

  const agentProfilePath = 'workflows/memory/profiles/agents/demo.memory-profile.yaml';
  const skillProfilePath = 'workflows/memory/profiles/skills/demo-skill.memory-profile.yaml';
  const agentExamplePath = 'workflows/memory/examples/agents/demo.memory.example.yaml';
  const skillExamplePath = 'workflows/memory/examples/skills/demo-skill.memory.example.yaml';
  ...
  writeJson(
    ...
    baseProfile('agent', 'agent:demo', 'agents/demo.agent.md', agentExamplePath)
  );
  writeJson(
    path.join(root, skillProfilePath),
    baseProfile('skill', 'skill:demo-skill', 'skills/demo-skill/SKILL.md', skillExamplePath)
  );
  writeJson(
    path.join(root, agentExamplePath),
    baseSnapshot('agent', 'agent:demo', agentProfilePath, 'agents/demo.agent.md')
  );
  writeJson(
    path.join(root, skillExamplePath),
    baseSnapshot('skill', 'skill:demo-skill', skillProfilePath, 'skills/demo-skill/SKILL.md')
  );

  writeJson(path.join(root, 'workflows', 'memory', 'registry', 'memory-registry.yaml'), {
    schema_version: '1.0.0',
...
describe('validate-memory', () => {
  ...
  it('fails when a skill profile is missing', () => {
    const root = buildTempRepo();
    fs.unlinkSync(
      path.join(root, 'workflows', 'memory', 'profiles', 'skills', 'demo-skill.memory-profile.yaml')
    );
