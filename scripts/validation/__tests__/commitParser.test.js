...
BREAKING CHANGE: Old API no longer supported`;
			...
			expect(result.type).toBe('feat');
			expect(result.body).toBe('This adds a new user profile page with all details.');
			expect(result.isBreaking).toBe(true);
			expect(result.footers['BREAKING CHANGE']).toBe('Old API no longer supported');
		});
	...
	describe('extractBreakingChanges', () => {
		it('extracts breaking changes', () => {
			const commits = [
				parseConventionalCommit('feat: feature with BREAKING CHANGE: old API removed'),
