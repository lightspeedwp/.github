...
describe("OpenSpec Status Labels", () => {
  ...
  describe("Label Configuration", () => {
    ...
    it("should have all 6 OpenSpec labels", () => {
      const openspecLabels = labelsConfig.filter((l) =>
        l.name.startsWith("openspec:"),
      );
      expect(openspecLabels).toHaveLength(6);
    });

    it("should have specification-pending label", () => {
      const label = labelsConfig.find(
    ...
    it("should have specification-in-progress label", () => {
      const label = labelsConfig.find(
        (l) => l.name === "openspec:specification-in-progress",
      );
      expect(label).toBeDefined();
    ...
    it("should have implementation-pending label", () => {
      const label = labelsConfig.find(
        (l) => l.name === "openspec:implementation-pending",
      );
      expect(label).toBeDefined();
      expect(label.color).toBe("C5DEF5"); // Blue
    ...
    it("should have implementation-complete label", () => {
      const label = labelsConfig.find(
        (l) => l.name === "openspec:implementation-complete",
      );
      expect(label).toBeDefined();
      expect(label.color).toBe("1A7F37"); // Green
  ...
  describe("Color Scheme", () => {
    it("should follow Blue→Yellow→Green lifecycle", () => {
      ...
      const specPending = labelsConfig.find(
        (l) => l.name === "openspec:specification-pending",
      );
      const specProgress = labelsConfig.find(
        (l) => l.name === "openspec:specification-in-progress",
      );
      const specComplete = labelsConfig.find(
        (l) => l.name === "openspec:specification-complete",
      );
      ...
      const implPending = labelsConfig.find(
        (l) => l.name === "openspec:implementation-pending",
      );
      const implProgress = labelsConfig.find(
        (l) => l.name === "openspec:implementation-in-progress",
      );
      const implComplete = labelsConfig.find(
        (l) => l.name === "openspec:implementation-complete",
      );
  ...
  describe("Naming Convention", () => {
    it("should follow naming pattern: openspec:{phase}-{status}", () => {
      ...
      const pattern =
        /^openspec:(specification|implementation)-(pending|in-progress|complete)$/;
  ...
  describe("Documentation", () => {
    it("should have clear descriptions for each label", () => {
      ...
      openspecLabels.forEach((label) => {
        ...
        const hasRelevantTerm = requiredTerms.some(
          (term) => description.includes(term) || label.name.includes(term),
        );
  ...
  describe("Lifecycle Tracking", () => {
    it("should enable tracking specification→implementation flow", () => {
      ...
      specProgression.forEach((status) => {
        expect(
          labels.find((l) => l.name === `openspec:specification-${status}`),
        ).toBeDefined();
      });

      implProgression.forEach((status) => {
        expect(
          labels.find((l) => l.name === `openspec:implementation-${status}`),
        ).toBeDefined();
      });
    });
