...
describe('sectionBody', () => {
  it('extracts content between headings', () => {
    ...
    expect(sectionBody(body, /^##\s+Linked issues\s*$/im)).toContain('Fixes #123');
  });
...
describe('hasIssueReference', () => {
  ...
  it('detects cross-repo references with closing keywords', () => {
    expect(hasIssueReference('Relates to lightspeedwp/.github#1079')).toBe(true);
    expect(hasIssueReference('resolves org/repo#100')).toBe(true);
  });

  it('detects full GitHub issue/PR URLs', () => {
    expect(hasIssueReference('https://github.com/org/repo/issues/123')).toBe(true);
    expect(hasIssueReference('https://github.com/org/repo/pull/456')).toBe(true);
  });
