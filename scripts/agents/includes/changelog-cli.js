...
function generateEntriesFromCommits(since = 'origin/develop..HEAD') {
	try {
		...
		const commits = [];
		const commitStrings = output.split('---END-COMMIT---\n').filter(s => s.trim());
		...
		let prNumber = null;
		try {
			const prBranch = execSync('git branch --show-current', { encoding: 'utf8' }).trim();
...
function interactiveEntry() {
	...
	return new Promise(resolve => {
		rl.question('Changelog entry: ', description => {
			rl.question('Section (added/fixed/changed/deprecated/removed/security): ', section => {
...
function showHelp() {
	...
	console.log('  generate    Generate entries from commits (default: since origin/develop)');
	...
	console.log('  --changelog <path> Path to CHANGELOG.md (default: ./CHANGELOG.md)\n');
}
...
function main() {
	...
	try {
		switch (parsed.command) {
			...
			case 'validate': {
				const { parseChangelog, validateChangelog } = require('./changelogUtils.cjs');
