...
class FeedbackProcessor {
  ...
  convertCodeRabbit(findings) {
    ...
    return findings.map(finding => ({
      id: this.generateId(TOOL_TYPES.CODERABBIT, finding.file, finding.line, finding.title),
  ...
  convertCodeQuality(findings) {
    ...
    return findings.map(finding => ({
      id: this.generateId(TOOL_TYPES.CODE_QUALITY, finding.path, finding.line, finding.message),
  ...
  convertCopilot(findings) {
    ...
    return findings.map(finding => ({
      id: this.generateId(TOOL_TYPES.COPILOT, finding.file, finding.line, finding.message),
  ...
  convertWordPress(findings) {
    ...
    return findings.map(finding => ({
      id: this.generateId(TOOL_TYPES.WORDPRESS, finding.file, finding.line, finding.message),
  ...
  extractCategoryFromCodeRabbit(finding) {
    ...
    if (title.includes('style') || title.includes('format') || title.includes('lint')) {
      return 'style';
    }
  ...
  mergeFindings(findings) {
    ...
    const severities = findings.map(f => f.severity).sort((a, b) => {
