...
function validateSchema(data, schema, path = 'root') {
    ...
    data.forEach((item, index) => {
      if (schema.items.oneOf) {
        ...
        for (const subSchema of schema.items.oneOf) {
          const subErrors = validateSchema(item, subSchema, `${path}[${index}]`);
        ...
        if (!valid) {
          errors.push(`${path}[${index}]: Item doesn't match any allowed schema`);
        }
      } else {
  ...
  if (schema.type === 'object') {
    ...
    if (schema.properties) {
      for (const [key, propSchema] of Object.entries(schema.properties)) {
        if (key in data) {
          const propErrors = validateSchema(data[key], propSchema, `${path}.${key}`);
  ...
  if (schema.type === 'string') {
    ...
    if (schema.pattern && !new RegExp(schema.pattern).test(data)) {
      errors.push(`${path}: String '${data}' doesn't match pattern ${schema.pattern}`);
    }
  }
...
function validateLabelsYml(filePath) {
  try {
    ...
    if (errors.length === 0) {
      core.info(`[yaml-validator] ✅ labels.yml is valid (${data.length} labels)`);
      return { valid: true, errors: [] };
    }

    core.error(`[yaml-validator] ❌ labels.yml validation failed with ${errors.length} error(s):`);
...
function validateIssueTypesYml(filePath) {
  try {
    ...
    if (errors.length === 0) {
      core.info(`[yaml-validator] ✅ issue-types.yml is valid (${data.issue_types.length} types)`);
      return { valid: true, errors: [] };
    }
...
function validateLabelerYml(filePath) {
  try {
    ...
    if (errors.length === 0) {
      ...
      core.info(`[yaml-validator] ✅ labeler.yml is valid (${labelCount} label rules)`);
      return { valid: true, errors: [] };
    }

    core.error(`[yaml-validator] ❌ labeler.yml validation failed with ${errors.length} error(s):`);
    errors.forEach((error) => core.error(`  - ${error}`));
...
function validateAllConfigs(config) {
  ...
  const allValid = results.labels.valid && results.issueTypes.valid && results.labeler.valid;

  if (allValid) {
    core.info('[yaml-validator] ✅ All configuration files are valid');
  } else {
    core.error('[yaml-validator] ❌ Some configuration files have validation errors');
  }
