...
async function runPlanner(options = {}) {
  ...
  try {
    ...
    log(
      `Found ${activeProjects.length} active projects: ${activeProjects.join(", ")}`,
    );
    ...
    if (dryRun) {
      ...
      log(
        `Would check for unassigned issues/PRs and propose project assignments`,
      );
      log(
        `Planner agent ready for activation once implementation is confirmed`,
      );
    } else {
