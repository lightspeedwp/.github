 ...
 * The 10-minute rate divides timely successful automatic runs by all eligible automatic attempts.
...
function readFromZip(buffer, fileName) {
  const entry = findCentralDirectoryEntry(buffer, fileName) ?? findLocalEntry(buffer, fileName);
  if (!entry) return null;
  const data = buffer.subarray(entry.dataStart, entry.dataStart + entry.compressedSize);
  return entry.method === 0 ? data.toString('utf8') : zlib.inflateRawSync(data).toString('utf8');
}

/**
 * Locate an entry through the central directory, which always records sizes,
 * including for streamed archives whose local headers defer them to a data descriptor.
 * @param {Buffer} buffer - Archive contents.
 * @param {string} fileName - Exact entry name to find.
 * @returns {{method: number, compressedSize: number, dataStart: number}|null}
 */
function findCentralDirectoryEntry(buffer, fileName) {
  let eocd = -1;
  ...
  for (let n = 0; n < entries && offset + 46 <= buffer.length; n += 1) {
...
/**
 * Fallback for archives without a central directory: scan sized local headers.
 * @param {Buffer} buffer - Archive contents.
 * @param {string} fileName - Exact entry name to find.
 * @returns {{method: number, compressedSize: number, dataStart: number}|null}
 */
function findLocalEntry(buffer, fileName) {
  let offset = 0;
  while (offset + 30 <= buffer.length && buffer.readUInt32LE(offset) === 0x04034b50) {
    ...
    if (name === fileName) return { method, compressedSize, dataStart };
