...
const typePatterns = {
  ...
  feature: {
    ...
    pattern:
      /feature|enhancement|improve|add support|new capability|would like/i,
  },
  documentation: {
    keywords: [
      "doc",
...
async function fetchIssue(issueNumber, repo, token) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        ...
        "User-Agent": "content-analysis-agent",
        ...
        res.on("end", () => {
          if (res.statusCode === 200) {
            try {
              ...
              reject(new Error(`Failed to parse issue data: ${e.message}`));
            }
          } else {
            reject(
              new Error(
...
async function main() {
  try {
    ...
    console.log(
      `Content Analysis Agent: Starting analysis of issue #${issueNumber}`,
    );

    // Fetch actual issue from GitHub API
    const issue = await fetchIssue(issueNumber, repo, args.token);
    ...
    console.log(
      `✓ Analysis complete: type=${type}, confidence=${confidence * 100}%`,
    );
    process.exit(0);
  } catch (error) {
