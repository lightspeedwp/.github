...
function displayHelp(action) {
  if (action && getActionDef(action)) {
    ...
    Object.entries(SCRIPT_REGISTRY).forEach(([action, def]) => {
      const icon = def.priority === 'high' ? '🔴' : def.priority === 'medium' ? '🟡' : '🟢';
      console.log(`${icon} ${action.padEnd(20)} ${def.description}`);
      console.log(`   Category: ${def.category} | Est. Time: ${def.estimatedTime}ms\n`);
    });

    console.log('EXAMPLES:');
    console.log('  orchestrator.js audit-metadata --repo lightspeedwp/.github --token $TOKEN');
    console.log('  orchestrator.js triage-all --repo lightspeedwp/.github --token $TOKEN');
...
function displayRegistry() {
  ...
  const totalTime = Object.values(SCRIPT_REGISTRY).reduce((sum, def) => sum + def.estimatedTime, 0);
  console.log(`📈 TOTALS`);
  console.log(`  Total Scripts: ${Object.keys(SCRIPT_REGISTRY).length}`);
  console.log(`  Total Est. Time: ${totalTime}ms (${(totalTime / 1000).toFixed(1)}s)`);
  console.log(`  Avg Per Script: ${(totalTime / Object.keys(SCRIPT_REGISTRY).length).toFixed(0)}ms\n`);
}
...
async function main() {
  ...
  console.log(`   Est. Time: ${(actionDef.estimatedTime / 1000).toFixed(1)}s\n`);
