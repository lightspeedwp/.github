...
describe("DoR/DoD Templates", () => {
  ...
  describe("detectTypeFromLabels()", () => {
    it("should detect type:bug from labels", () => {
      const labels = [{ name: "type:bug" }, { name: "priority:high" }];
    ...
    it("should return null if no type label present", () => {
      const labels = [{ name: "priority:high" }, { name: "area:ci" }];
...
- [ ] Fix implemented`,
      ...
      expect(dorDodTemplates.detectTypeFromLabels(issue.labels)).toBe(
        "type:bug",
      );
    });
