...
class GitHubAPIClient {
  ...
  async getIssue(owner, repo, issueNumber) {
    ...
    return { status: 200, data: { ...fixtures.issues.issueWithLabels, number: issueNumber } };
  }

  async createIssue(owner, repo, title, body, labels = []) {
    this.recordRequest('POST', `/repos/${owner}/${repo}/issues`, { title, body, labels });
    return {
      status: 201,
      data: {
  ...
  async updateIssue(owner, repo, issueNumber, updates) {
    this.recordRequest('PATCH', `/repos/${owner}/${repo}/issues/${issueNumber}`, updates);
    return {
      status: 200,
      data: { ...fixtures.issues.issueWithLabels, ...updates },
    };
  }

  async addLabels(owner, repo, issueNumber, labels) {
    this.recordRequest('POST', `/repos/${owner}/${repo}/issues/${issueNumber}/labels`, { labels });
    return {
      status: 200,
  ...
  async removeLabel(owner, repo, issueNumber, label) {
    this.recordRequest('DELETE', `/repos/${owner}/${repo}/issues/${issueNumber}/labels/${label}`);
    return { status: 204 };
  }

  async listIssueLabels(owner, repo, issueNumber) {
    this.recordRequest('GET', `/repos/${owner}/${repo}/issues/${issueNumber}/labels`);
  ...
  async createLabel(owner, repo, name, color, description = '') {
    this.recordRequest('POST', `/repos/${owner}/${repo}/labels`, { name, color, description });
    return {
      status: 201,
      data: {
        id: Math.random(),
  ...
  async updateLabel(owner, repo, labelName, updates) {
    this.recordRequest('PATCH', `/repos/${owner}/${repo}/labels/${labelName}`, updates);
  ...
  describe('Issue Operations', () => {
    ...
    describe('createIssue', () => {
      it('creates issue with title and body', async () => {
        const response = await client.createIssue(owner, repo, 'New issue', 'Issue description');
      ...
      it('creates issue with labels', async () => {
        ...
        const response = await client.createIssue(owner, repo, 'Bug report', 'Found a bug', labels);
        expect(response.status).toBe(201);
      ...
      it('creates issue without labels', async () => {
        const response = await client.createIssue(owner, repo, 'Basic issue', 'No labels');
        expect(response.status).toBe(201);
    ...
    describe('updateIssue', () => {
      it('updates issue title', async () => {
        const response = await client.updateIssue(owner, repo, 1001, { title: 'Updated title' });
        expect(response.status).toBe(200);
  ...
  describe('Label Operations', () => {
    describe('addLabels', () => {
      it('adds single label to issue', async () => {
        const response = await client.addLabels(owner, repo, 1001, ['type:bug']);
        expect(response.status).toBe(200);
    ...
    describe('removeLabel', () => {
      it('removes label from issue', async () => {
        const response = await client.removeLabel(owner, repo, 1001, 'type:bug');
    ...
    describe('createLabel', () => {
      it('creates new label in repository', async () => {
        const response = await client.createLabel(owner, repo, 'custom:label', 'ff6b6b', 'Custom label');
        expect(response.status).toBe(201);
      ...
      it('creates label without description', async () => {
        const response = await client.createLabel(owner, repo, 'quick-label', 'ffffff');
        expect(response.status).toBe(201);
  ...
  describe('Search Operations', () => {
    describe('searchIssues', () => {
      it('searches issues by query', async () => {
        const response = await client.searchIssues(owner, repo, 'type:bug state:open');
        expect(response.status).toBe(200);
