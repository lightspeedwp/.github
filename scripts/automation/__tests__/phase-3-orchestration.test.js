...
describe('Phase 3: Workflow Orchestration', () => {
  describe('Phase State Machine', () => {
    describe('State Transitions', () => {
      it('should allow specification:pending → specification:in-progress', () => {
        expect(phaseStateMachine.isValidTransition(
    ...
    describe('Progression Detection', () => {
      it('should detect forward progression', () => {
        expect(phaseStateMachine.isProgression(
          phaseStateMachine.STATES.SPECIFICATION_PENDING,
          phaseStateMachine.STATES.SPECIFICATION_IN_PROGRESS
        )).toBe(true);
      ...
      it('should detect rollback', () => {
        expect(phaseStateMachine.isRollback(
          phaseStateMachine.STATES.SPECIFICATION_IN_PROGRESS,
          phaseStateMachine.STATES.SPECIFICATION_PENDING
        )).toBe(true);
    ...
    describe('Phase and Step Detection', () => {
      it('should extract phase from state', () => {
        expect(phaseStateMachine.getPhase(phaseStateMachine.STATES.SPECIFICATION_PENDING))
          .toBe('specification');
        expect(phaseStateMachine.getPhase(phaseStateMachine.STATES.IMPLEMENTATION_PENDING))
          .toBe('implementation');
      });

      it('should extract step from state', () => {
        expect(phaseStateMachine.getStep(phaseStateMachine.STATES.SPECIFICATION_PENDING))
          .toBe('pending');
        expect(phaseStateMachine.getStep(phaseStateMachine.STATES.SPECIFICATION_IN_PROGRESS))
          .toBe('in-progress');
        expect(phaseStateMachine.getStep(phaseStateMachine.STATES.SPECIFICATION_COMPLETE))
          .toBe('complete');
      });
    });
  ...
  describe('Label Validator', () => {
    ...
    describe('Label Extraction', () => {
      it('should extract OpenSpec label', () => {
        const labels = ['openspec:specification-in-progress', 'status:in-progress', 'type:bug'];
        expect(labelValidator.getOpenSpecLabel(labels))
          .toBe('openspec:specification-in-progress');
      });
      ...
      it('should extract type label', () => {
        const labels = ['openspec:specification-pending', 'type:feature', 'priority:important'];
        expect(labelValidator.getTypeLabel(labels)).toBe('type:feature');
      });

      it('should extract status labels', () => {
        const labels = ['status:in-progress', 'status:needs-review', 'type:bug'];
    ...
    describe('Transition Validation', () => {
      ...
      it('should reject invalid OpenSpec transition', () => {
        ...
        const result = labelValidator.validateTransition(current, next);
        expect(result.valid).toBe(false);
        expect(result.conflicts.some(c => c.includes('Invalid OpenSpec'))).toBe(true);
      });

      it('should track added and removed labels', () => {
        const current = ['openspec:specification-pending', 'status:needs-planning'];
        const next = ['openspec:specification-in-progress', 'status:in-progress'];
        const result = labelValidator.validateTransition(current, next);
  ...
  describe('Audit Logger', () => {
    describe('Audit Entry Creation', () => {
      ...
      it('should include label changes in details', () => {
        ...
        expect(entry.details.added).toContain('openspec:specification-in-progress');
        expect(entry.details.removed).toContain('openspec:specification-pending');
      });
    });
    ...
    describe('Audit Summary', () => {
      ...
      it('should track label changes in summary', () => {
        ...
        const summary = auditLogger.generateAuditSummary(entries);
        expect(summary.labelChanges.added['openspec:specification-in-progress']).toBe(2);
      });
    });
    ...
    describe('Audit Filtering', () => {
      ...
      it('should filter by issue number', () => {
        ...
        const filtered = auditLogger.filterAuditLog(entries, { issueNumber: 123 });
        expect(filtered).toHaveLength(1);
  ...
  describe('Integration: Full Phase Progression', () => {
    it('should validate complete specification → implementation workflow', () => {
      ...
      const step1 = labelValidator.validateTransition(startLabels, inProgressLabels);
      ...
      const step2 = labelValidator.validateTransition(inProgressLabels, completeLabels);
      ...
      const step3 = labelValidator.validateTransition(completeLabels, implPendingLabels);
