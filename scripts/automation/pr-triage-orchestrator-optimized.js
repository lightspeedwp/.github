...
/**
 * PR Triage Orchestrator - Phase 2C Optimized
 *
 * Comprehensive PR metadata management tool with Phase 2C optimizations:
 * - Caching for stable queries (milestone/assignee lookups)
 * - Batch issue metadata fetches with parallel requests
 * - Native fetch for better performance
 * - Rate limit handling
 *
 * Expected improvement: 10-15% over baseline
 *
 * Usage:
 *   node pr-triage-orchestrator-optimized.js --dry-run [--limit=N]
 *   node pr-triage-orchestrator-optimized.js --auto [--assignee=ID] [--milestone=ID]
 */
...
console.log(`🔍 PR Triage Orchestrator (Phase 2C Optimized)`);
console.log(
...
/**
 * Cache milestone lookups
 */
async function getMilestoneByName(name) {
  const cacheKey = `milestone:${name}`;
  ...
  const milestone = milestones.data.find((m) => m.title === name);
  if (milestone) {
...
/**
 * Check if PR has linked issue (with caching)
 */
async function getPRLinkedIssues(prNumber) {
  const cacheKey = `pr-linked-issues:${prNumber}`;
  const cached = cache.get(cacheKey);
  if (cached !== null) {
    return cached;
  }
  ...
  try {
    ...
    cache.set(cacheKey, linkedIssues);
    return linkedIssues;
  } catch (error) {
...
async function updatePRMetadata(pr, _linkedIssueNumber = null) {
  ...
  try {
    ...
    if (mode === "auto") {
      ...
      // Update milestone
      if (!pr.milestone) {
        const milestone = await getMilestoneByName(config.milestone);
        if (milestone) {
