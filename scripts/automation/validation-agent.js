...
const validationRules = {
  titleQuality: {
    ...
    check: (title) => {
      if (!title || title.length < 5)
        return { pass: false, message: "Title too short" };
  ...
  bodyQuality: {
    ...
    check: (body) => {
      if (!body || body.length < 20)
        return { pass: false, message: "Description too short" };
      if (body.includes("TODO") || body.includes("FIXME"))
        return { pass: true, message: "Has implementation notes" };
  ...
  typeLabel: {
    ...
    check: (labels) => {
      ...
      if (typeLabels.length === 0)
        return { pass: false, message: "Missing type label" };
      if (typeLabels.length > 1)
        return { pass: false, message: "Multiple type labels found" };
  ...
  statusLabel: {
    ...
    check: (labels) => {
      ...
      if (statusLabels.length === 0)
        return { pass: false, message: "Missing status label" };
      if (statusLabels.length > 1)
        return { pass: false, message: "Multiple status labels found" };
  ...
  priorityLabel: {
    ...
    check: (labels) => {
      ...
      if (priorityLabels.length === 0)
        return { pass: false, message: "Missing priority label" };
      if (priorityLabels.length > 1)
        return { pass: false, message: "Multiple priority labels found" };
  ...
  typeBodyAlignment: {
    ...
    check: (type, body) => {
      ...
      const checks = {
        ...
        feature:
          body.toLowerCase().includes("enhance") ||
          body.toLowerCase().includes("add"),
        security:
          body.toLowerCase().includes("security") ||
          body.toLowerCase().includes("vulnerable"),
        performance:
      ...
      if (checks[type] !== undefined) {
        ...
        return {
          pass: false,
          message: "Type does not align with body content",
        };
      }
  ...
  noOffensiveLanguage: {
    ...
    check: (body) => {
      ...
      const found = offensive.filter((word) =>
        body.toLowerCase().includes(word),
      );
      if (found.length > 0) {
        return {
          pass: false,
          message: `Offensive language detected: ${found.join(", ")}`,
        };
      }
...
function validateIssue(title, body, type, labels) {
  ...
  if (priorityCheck.pass) passCount++;
  else failCount++;
  ...
  if (alignmentCheck.pass) passCount++;
  else failCount++;
  ...
  if (languageCheck.pass) passCount++;
  else failCount++;
...
async function main() {
  try {
    ...
    const body =
      "This is a detailed issue body with proper formatting and clear description.";
    const type = "bug";
    ...
    console.log(
      "::set-output name=passed::" + (validation.overall_status !== "fail"),
    );

    console.log(`✓ Validation complete: ${validation.overall_status}`);
    console.log(
      `  Passed: ${validation.pass_count}, Failed: ${validation.fail_count}`,
    );
