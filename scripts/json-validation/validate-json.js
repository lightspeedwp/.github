...
// validate-json.js
//
// Comprehensive JSON linting and validation script for LightSpeedWP
//
// Features:
...
function validateSyntax(files) {
  ...
  for (const file of files) {
    try {
      ...
      JSON.parse(content); // Basic syntax check
      log.debug(`✓ ${file}`);
    } catch (error) {
      invalid++;
...
async function validateSchema(files) {
  ...
  if (invalid > 0) {
    ...
    if (config.errorsFormat === 'json') {
      ...
      const errorText = allErrors.map(err =>
        `${err.file} → ${err.path}: ${err.message} (${err.keyword})`
      ).join('\n');
