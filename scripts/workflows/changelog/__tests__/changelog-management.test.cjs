...
const TEST_DIR = path.join(__dirname, '../../../..', '.github/tmp/changelog-mgmt-test');
...
test('Validates standard changelog sections', () => {
  const validSections = ['### Added', '### Fixed', '### Changed', '### Removed', '### Deprecated', '### Security'];
  const changelog = `# Changelog
...
`;
  ...
  const content = fs.readFileSync(CHANGELOG_PATH, 'utf8');
  const foundSections = validSections.filter(section => content.includes(section));
...
test('Enforces label restrictions (no meta:no-changelog for features)', () => {
  ...
  assert(restrictedTypes.has('type:feature'), 'Feature type must require changelog');
  assert(restrictedTypes.has('type:bug'), 'Bug type must require changelog');
  assert(restrictedTypes.has('type:security'), 'Security type must require changelog');
});
...
test('Allows meta:no-changelog for non-critical types', () => {
  ...
  allowedToSkip.forEach(type => {
    const isRestricted = new Set([
      'type:feature', 'type:bug', 'type:performance', 'type:security', 'type:release', 'type:hotfix'
    ]).has(type);
...
test('Recognizes Dependabot authors for skip', () => {
  ...
  assert(dependabotAuthors.includes('dependabot[bot]'), 'Should recognize dependabot[bot]');
  assert(dependabotAuthors.includes('app/dependabot'), 'Should recognize app/dependabot');
});
...
`;
  ...
  assert(base.includes('## [Unreleased]'), 'Base should have Unreleased section');
...
test('Formats commit message correctly', () => {
  ...
  assertIncludes(expectedMsg, '[skip ci]', 'Commit should include [skip ci] flag');
  assertIncludes(expectedMsg, `PR #${prNumber}`, 'Commit should reference PR number');
  assertIncludes(expectedMsg, 'chore(changelog)', 'Commit should have correct prefix');
});
...
`;
  ...
  const content = fs.readFileSync(CHANGELOG_PATH, 'utf8');
  const hasValidSections = ['### Added', '### Fixed'].some(section => content.includes(section));
...
`;
  ...
  const content = fs.readFileSync(CHANGELOG_PATH, 'utf8');
  const hasValidStructure = content.includes('# Changelog') && content.includes('## [Unreleased]');
...
test('Rejects conflicting labels', () => {
  ...
  const hasConflict = conflictingLabels.has_needs_changelog && conflictingLabels.has_no_changelog;
