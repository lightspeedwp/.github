...
const CONTEXTS = ['github-control-plane', 'wordpress-plugin', 'wordpress-theme'];
const OUTPUT_DIR = path.join(__dirname, '../../..', '.githu./.github/reports/metrics');
...
class MetricsCollectionOrchestrator {
  ...
  getContexts() {
    ...
    throw new Error(`Unknown context: ${this.context}. Valid options: ${CONTEXTS.join(', ')}`);
  }
  ...
  async collectMetrics(context) {
    ...
    try {
      ...
      if (this.dry) {
        this.log(`[DRY RUN] Would execute: node ${METRICS_AGENT} ${configPath}`);
      ...
      this.log(`Failed to collect metrics for ${context}: ${error.message}`, 'error');
  ...
  async collect() {
    try {
      ...
      this.log(`Collecting metrics for ${contexts.length} context(s): ${contexts.join(', ')}`);
      ...
      const resultsFile = path.join(this.outputDir, `collection-${this.getTimestamp()}.json`);
      ...
      if (process.env.GITHUB_OUTPUT) {
        ...
        fs.appendFileSync(process.env.GITHUB_OUTPUT, `collection_timestamp=${timestamp}\n`);
        fs.appendFileSync(process.env.GITHUB_OUTPUT, `results_file=${resultsFile}\n`);
        fs.appendFileSync(process.env.GITHUB_OUTPUT, `successful=${this.results.summary.successful}\n`);
        fs.appendFileSync(process.env.GITHUB_OUTPUT, `failed=${this.results.summary.failed}\n`);
      }

      if (this.results.summary.failed > 0) {
        throw new Error(`${this.results.summary.failed} context(s) failed to collect metrics`);
      }
