...
describe('MetricsCollectionOrchestrator', () => {
  ...
  describe('getContexts', () => {
    it('returns all contexts when context is "all"', () => {
      const orchestrator = new MetricsCollectionOrchestrator({ context: 'all' });
      const contexts = orchestrator.getContexts();
      expect(contexts).toEqual(['github-control-plane', 'wordpress-plugin', 'wordpress-theme']);
    });

    it('returns single context when context is specified', () => {
      const orchestrator = new MetricsCollectionOrchestrator({ context: 'github-control-plane' });
      const contexts = orchestrator.getContexts();
      expect(contexts).toEqual(['github-control-plane']);
    });

    it('throws error for invalid context', () => {
      const orchestrator = new MetricsCollectionOrchestrator({ context: 'invalid' });
  ...
  describe('validateSetup', () => {
    it('validates metrics agent exists', () => {
      ...
      expect(() => orchestrator.validateSetup()).toThrow(/Metrics agent not found/);
    });
    ...
    it('validates config directory exists', () => {
      ...
      expect(() => orchestrator.validateSetup()).toThrow(/Config directory not found/);
    });
  ...
  describe('ensureOutputDir', () => {
    it('creates output directory if it does not exist', () => {
      ...
      expect(fs.mkdirSync).toHaveBeenCalledWith(orchestrator.outputDir, { recursive: true });
    });
  ...
  describe('collectMetrics', () => {
    ...
    it('handles dry run mode', async () => {
      ...
      expect(orchestrator.results.contexts['github-control-plane'].status).toBe('dry-run');
      expect(orchestrator.results.summary.successful).toBe(1);
    });
    ...
    it('handles missing config file', async () => {
      ...
      expect(orchestrator.results.contexts['github-control-plane'].status).toBe('failed');
    ...
    it('handles execution errors', async () => {
      ...
      expect(orchestrator.results.contexts['github-control-plane'].status).toBe('failed');
      expect(orchestrator.results.contexts['github-control-plane'].error).toBe('Command failed');
      expect(orchestrator.results.summary.failed).toBe(1);
    });
  ...
  describe('log', () => {
    ...
    it('respects verbose flag', () => {
      jest.spyOn(console, 'log').mockImplementation();
      const orchestrator = new MetricsCollectionOrchestrator({ verbose: false });
