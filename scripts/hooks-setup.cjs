...
function setupHook() {
  ...
  if (!isGitRepo()) {
    console.error(
      "❌ Not a Git repository. Run this command from the repo root.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      "❌ Hook is not executable. Try running: chmod +x " + TARGET_HOOK,
    );
    process.exit(1);
  }
  ...
  console.log(
    "  1. Create a branch with an invalid name: git checkout -b my-invalid-branch",
  );
