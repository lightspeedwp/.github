...
_Updated 17 Oct 2025 to include **BugHerd families** (`type:_`, `page:_`, `device:_`, `layout:_`, `theme:_`, `block:_`, `template:_`, `template-part:_`, `woo:_`, `to:_`, `phase:_`, `size:_`) as **org-wide GitHub labels**. Workflows and labeler patterns remain compatible — extend `.github/labeler.yml`to map paths to new`comp:_`/`block:_`/`template:_` labels.\*
