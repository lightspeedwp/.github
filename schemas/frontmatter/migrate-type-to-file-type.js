...
function main() {
  ...
  console.log(`${colors.cyan}║  Frontmatter Type → File_Type Migration║${colors.reset}`);
  console.log(`${colors.cyan}╚════════════════════════════════════════╝${colors.reset}\n`);
  ...
  console.log(`${colors.yellow}Found ${filesToMigrate.length} files that need migration${colors.reset}\n`);
  ...
  filesToMigrate.forEach(file => {
    ...
    try {
      ...
      if (changed) {
        ...
        console.log(`${colors.yellow}○${colors.reset} ${relativePath} (no changes needed)`);
      }
    } catch (error) {
      errorCount++;
  ...
  if (DRY_RUN) {
    ...
    console.log(`  3. Commit changes: git commit -am "fix(frontmatter): migrate type to file_type field"`);
  }
