 ...
 * Types: authorized branch types (mirrors ALLOWED_TYPES in
 * scripts/validation/validate-branch-name.cjs, the canonical list).
...
// Branches exempt from validation (long-lived protected branches).
// Mirrors PROTECTED_BRANCHES in scripts/validation/validate-branch-name.cjs,
// which is the canonical validator. Without this exemption the
// branch-name-validation workflow fails on every push to develop/main.
const PROTECTED_BRANCHES = new Set(['main', 'develop']);

// Bot-owned branches (machine-generated, cannot be renamed).
// Mirrors BOT_PREFIXES in scripts/validation/validate-branch-name.cjs.
const BOT_PREFIXES = /^(dependabot|renovate)\//;

// Release branches may also use semantic versioning (release/v1.2.3,
// release/1.2.3, release/v1.2.3-rc1), as documented in
// docs/BRANCHING_STRATEGY.md. Mirrors BRANCH_PATTERN_RELEASE_SEMVER in
// scripts/validation/validate-branch-name.cjs.
const RELEASE_SEMVER_PATTERN = /^release\/v?\d+\.\d+\.\d+(-[a-z0-9]+)*$/;

// Pattern for extracting type, scope, title (must be exact case - lowercase)
const PARSE_PATTERN = /^([a-z0-9]+)\/([a-z0-9]+(?:-[a-z0-9]+)*)-([a-z0-9]+(?:-[a-z0-9]+)*)$/;
...
/**
 * Validate a branch name against LightSpeed naming rules.
 *
 * Accepts authorized {type}/{scope}-{title} names and release versions with
 * three dot-separated numeric components, an optional lowercase v, and optional
 * hyphen-separated lowercase alphanumeric suffixes (e.g., release/v1.2.3-rc1).
 * The exact names main and develop and the prefixes dependabot/ and renovate/
 * are exempt from further validation.
 *
 * @param {string} branchName - Name to validate after trimming surrounding whitespace.
 *   Empty strings and non-string inputs return an invalid result.
 * @returns {Object} Result with valid, type, scope, title, errors, and suggested_name.
 *   Standard names populate type, scope, and title, splitting at the last hyphen.
 *   Release versions set type to 'release' and leave scope and title null;
 *   exempt names leave all three null. Invalid names leave all three null and
 *   return the first error code in errors, with a suggested_name when available.
 *   Valid names have an empty errors array and a null suggested_name.
 */
function validateBranchName(branchName) {
  ...
  // Skip validation for protected long-lived branches
  if (PROTECTED_BRANCHES.has(trimmed)) {
    result.valid = true;
    return result;
  }

  // Skip validation for bot-owned branches (dependabot/renovate):
  // machine-generated names that cannot follow the human pattern.
  if (BOT_PREFIXES.test(trimmed)) {
    result.valid = true;
    return result;
  }
  ...
  // Semantic-version release branches have no scope/title to parse
  if (RELEASE_SEMVER_PATTERN.test(trimmed)) {
    result.valid = true;
    result.type = 'release';
    return result;
  }
  ...
  if (!parsed) {
    ...
    if (hyphenIndex === -1 || hyphenIndex === 0 || hyphenIndex === potentialRest.length - 1) {
    ...
    const scopeValidation = validateScopeOrTitle(scope);
    const titleValidation = validateScopeOrTitle(title);
