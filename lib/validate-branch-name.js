...
// Mirrors BRANCH_PATTERN_RELEASE_SEMVER in scripts/validation/validate-branch-name.cjs,
// which is the canonical validator. Release branches accept EITHER:
// 1. Semantic version: release/v1.2.3 or release/1.2.3
// 2. Standard format: release/{scope}-{title} (handled by the generic pattern below)
...
function validateBranchName(branchName) {
  ...
  // Accept release branches in semantic versioning format (e.g., release/v1.0.0).
  // Standard release/{scope}-{title} names are handled by the generic pattern below.
  if (BRANCH_PATTERN_RELEASE_SEMVER.test(trimmed)) {
    result.valid = true;
    return result;
  }
  ...
  // Protected branches are exact-name exceptions in both local and remote validation.
  if (PROTECTED_BRANCHES.has(trimmed)) {
    result.valid = true;
    return result;
  }
...
/**
 * Format a branch validation result for human-readable output.
 *
 * @param {string} branchName - The branch name that was validated.
 * @param {{ errors: string[], suggested_name: string | null }} validationResult - The validation result to format.
 * @returns {string} A status message, including a suggested branch name when available.
 */
