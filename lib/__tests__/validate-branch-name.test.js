...
describe('validateBranchName', () => {
  describe('Valid Branch Names (38 Canonical Types)', () => {
    ...
    test('should pass for release semantic-version format', () => {
      for (const name of ['release/v1.0.0', 'release/1.2.3', 'release/v2.0.0-rc1']) {
        expect(validateBranchName(name)).toMatchObject({
          valid: true,
          type: 'release',
          scope: null,
          title: null,
  ...
  describe('Edge Cases', () => {
    test('should pass for protected branch main', () => {
      const result = validateBranchName('main');
      expect(result.valid).toBe(true);
      expect(result.errors).toEqual([]);
    });

    test('should pass for protected branch develop', () => {
  ...
  describe('Error Messages', () => {
    ...
    test('should derive invalid-type message from the canonical type list', () => {
      const result = validateBranchName('feature/user-auth');
      const message = formatErrorMessage('feature/user-auth', result);
      expect(message).toContain(`Allowed types (${AUTHORIZED_TYPES.length})`);
      for (const type of AUTHORIZED_TYPES) {
        expect(message).toContain(type);
      }
    });

    test('should derive malformed-pattern message from the canonical type list', () => {
      const result = validateBranchName('badname');
      const message = formatErrorMessage('badname', result);
      expect(message).toContain(`one of ${AUTHORIZED_TYPES.length} types`);
    });
