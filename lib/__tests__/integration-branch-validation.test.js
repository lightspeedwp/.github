...
const PR_TEMPLATE_CONFIG_PATH = path.join(
  REPOSITORY_ROOT,
...
const prTemplateConfig = yaml.load(fs.readFileSync(PR_TEMPLATE_CONFIG_PATH, 'utf8'));
...
describe('T131-T135: Template & Label Routing Integration Tests', () => {
  ...
  describe('T134: All 38 types template/label coverage', () => {
    ...
    test('all supported branch routes match the authoritative template mapping', () => {
      for (const [type, { template }] of Object.entries(branchTypesConfig.branch_types)) {
        const filename = `${template}.md`;
        expect(prTemplateConfig.routes[`${type}/`]).toBe(filename);
        expect(prTemplateConfig.available_templates).toContain(filename);
        expect(
          fs.existsSync(path.join(REPOSITORY_ROOT, '.github/PULL_REQUEST_TEMPLATE', filename))
        ).toBe(true);
      }
    });
