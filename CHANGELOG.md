...
- **Requirements Quality Checklist Phase 5** — Added audience-specific guidance for authors, peer reviewers, stakeholder approvers and integration reviewers, plus an audience selection guide. (#3361)
...
- **Label Remediation Executed** — Remediated 83 bare-label violations with canonical labels. (#2523)

- **Node.js 24 Upgrade Completed** — Moved the control plane to Node.js 24 with aligned configs. (#2447)

- **Agent Spec Audit Phase 4 Completed** — Added validation workflow, pre-commit check and developer guide. (#2526)

- **Bulk Label Remediation Completed** — Remediated legacy bare labels across issues and pull requests. (#2476)

- **Pull Request Governance Added** — Added validation for linking, milestones and templates. (#2418)

- **Changelog Safety Audit Added** — Added a seven-layer audit engine with workflow integration. (#2379)

- **Alerts Cleared and Actions Bumped** — Resolved open alerts and moved stale actions to current majors. (#2844)
...
- **Agent Consolidation Phase 7 Fixed** — Aligned decision criteria to require Critical Blockers == 0 with T080-Defer follow-up. (#3116)

- **Pre-commit Lint Scoped to Changes** — Limited the markdown check to lines the commit touched. (#3159)

- **Test Debt Triaged** — Triaged pre-existing test failures and fixed verified defects. (#3311)

- **Lint Config Repaired** — Declared missing globals so linting unblocked pushes. (#2847)

- **Undeclared Globals Fixed** — Fixed bare calls that would have thrown at runtime. (#2847)

- **Required Check Restored for Pushes** — Extended the template check trigger to cover pushes. (#2847)

- **GitHub Client Loading Fixed** — Switched scripts to dynamic import through a shared helper. (#2846)

- **Workflow Parse Errors Fixed** — Fixed six workflow files rejected at parse time. (#2844)

- **Labelling Check Link Fixed** — Declared the missing job link with always-run semantics. (#2844)

- **Branch Name Injection Fixed** — Passed branch names through the environment with quoting. (#2844)

- **Missing Library Declared** — Declared the transitive library at a version with the expected export. (#2844)

- **Repository Context Fixed for Command Line** — Set the repository setting so lookup commands resolve without a clone. (#2844)

- **Workflow Hardening Completed** — Moved secrets into environment blocks with input validation. (#2641)

- **Test Helpers Deduplicated** — Replaced duplicated mocks with shared helper units. (#2456)

- **Markdown Ignore Patterns Unified** — Made linting consume ignore patterns from the canonical file. (#2448)

### Removed

- **Project Sync Workflows Unified** — Merged two sync workflows into one with dry-run support. (#1408)
