...
- **Babel Dependency Incompatibility — Issue #2582** — Fixed npm ERESOLVE errors blocking all CI checks by upgrading Babel toolchain from mixed 7.x/8.x to unified 8.x versions. Updated `@babel/core` from 7.29.7 to 8.0.1, `@babel/preset-env` from 7.28.5 to 8.0.2, and `@babel/preset-typescript` from 7.29.7 to 8.0.1. Added `legacy-peer-deps=true` to `.npmrc` to allow forward-compatible Babel 7.x syntax plugins with Babel 8.x core. Verified with full test suite (176 suites, 3,498 tests passing) and ESLint validation. ([#2582](https://github.com/lightspeedwp/.github/issues/2582), [PR #2583](https://github.com/lightspeedwp/.github/pull/2583))
