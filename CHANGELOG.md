...
- **PRD Agent Folder Consolidation — Phase 4: External Registry Updates & Documentation Fixes** — Completed Phase 4 of the PRD Agent consolidation initiative finalizing agent loadability for Claude Code and GitHub Copilot. Phase 4 deliverables include: (1) **Agent Frontmatter Updates** — Updated `agents/prd-agent/claude/agent.md` and `agents/prd-agent/copilot/agent.md` with realistic, loadable tool configurations matching actual MCP server integrations (Linear, GitHub, Google Workspace) instead of placeholder `["all"]` configurations; validates against SC-002 and SC-003 acceptance criteria for real tool availability; (2) **Memory Registry Repointing** — Updated `workflows/memory/registry/memory-registry.yaml` (line 85) and `workflows/memory/registry/inventory-lock.json` (line 20) to repoint `mode-prd` agent entry from deprecated `agents/mode-prd.agent.md` to canonical location `agents/prd-agent/copilot/agent.md`; ensures registry validation passes and maintains PRD agent tracking across consolidation; (3) **Documentation Link Repairs** — Fixed 8 broken references in `docs/AGENT-INDEX.md` that pointed to deprecated `agents/mode-prd.agent.md`, updating all links to target new canonical location; eliminates 100% of dead documentation links post-consolidation; (4) **Agent Deprecation** — Safely removed `agents/mode-prd.agent.md` after registry repointing, completing transition from distributed mode-specific agents to unified consolidated structure. **Validation Results**: Agent Specification Validation (PASSED ✓), PR Template Validation (PASSED ✓), Registry Validation Script passes with updated file locations. **Impact**: Consolidates 28 consolidated skills from separate mode-specific agents into single canonical location; enables agent loadability for Claude Code and Copilot platforms; establishes single source of truth for PRD agent definitions; completes 4-phase consolidation roadmap (Phase 1: specs, Phase 2: implementations, Phase 3: platform integrations, Phase 4: external registry sync). **Related**: Extends agent consolidation work from earlier phases; precursor for platform integration in Phase 5. ([PR #2892](https://github.com/lightspeedwp/.github/pull/2892))
