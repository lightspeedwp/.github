...
- **Agent Specification Generator CLI — Phase 5 Goal 2** — Interactive CLI tool for scaffolding new agent specifications with real-time validation and batch processing support. Deliverables: (1) **Interactive CLI Tool** (`scripts/create-agent-spec.js`, 600 LOC) — Prompts for agent metadata (name, description, category, author, purpose, version, status) with real-time validation; supports three modes: interactive (`npm run create:agent`), pre-filled category (`npm run create:agent -- --category governance`), and batch generation from JSON (`npm run create:agent -- --batch agents.json`); validates all inputs against schema requirements (kebab-case names, 10-200 character descriptions, semantic versioning, valid categories/statuses); generates valid `.agent.md` spec files with complete YAML frontmatter; creates implementation directory structure with SKILL.md and README.md templates. (2) **Agent Specification Template** (`scripts/templates/agent.template.md`) — Complete frontmatter template with 12 placeholder variables for all required fields (name, description, file_type, category, status, version, created_date, last_updated, author, implementation); includes structured markdown sections for Purpose, Core Responsibilities, Key Features, Operating Modes, and Implementation Reference. (3) **CLI Test Suite** (`scripts/__tests__/create-agent-spec.test.js`, 19 tests, 100% pass rate) — Comprehensive validation covering: template file existence and completeness, script executability and CLI interface, all validator functions (name, description, category, status, author, version), category/status definitions, template generation with placeholder replacement, spec file writing and directory creation, batch processing from realistic JSON formats, npm script registration; tests isolated and non-breaking. (4) **Integration** — npm scripts: `create:agent` (interactive CLI), `test:create-agent-spec` (CLI tests); updated Phase 5 test suite runner (`.github/scripts/__tests__/run-all-tests.sh`) to include CLI validation; 19/19 CLI tests + 70+ Phase 5 total tests passing. **Validator Functions**: 6 comprehensive validators covering agent names (kebab-case, 3-50 chars), descriptions (10-200 chars), categories (10 valid values), statuses (active/draft/deprecated), authors (2+ chars), versions (semantic format). **Features**: Real-time input validation with error messages, default values for common fields, pre-filled category support for faster entry, batch processing for bulk agent creation, automatic directory structure generation, semantic versioning requirement enforcement, YAML frontmatter validation. **Success Metrics**: Interactive flow completes in <2 minutes, all 10 required frontmatter fields auto-populated, batch processing tested with realistic JSON, 100% test coverage of CLI components. ([PR #2620](https://github.com/lightspeedwp/.github/pull/2620))
