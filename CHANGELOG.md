...
- **Metrics Rebase Conflict Resolution** — The metrics push retry aborted on snapshot conflicts and failed the job. The retry loop now lives in `scripts/workflows/push-with-retry.sh` with BATS coverage, resolves snapshot-only conflicts to the current run, and the workflow queues scheduled runs via `queue: max`. ([PR #3394](https://github.com/lightspeedwp/.github/pull/3394))
