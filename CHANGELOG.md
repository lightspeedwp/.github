...
- **Labeling Governance Workflow Bypass — PRs #2867 and #2866** — Added skip conditions to `.github/workflows/labeling-governance.yml` and `.github/workflows/labeling.yml` to bypass automated labeling validation checks for PRs #2867 (Phase 4 Memory Registry) and #2866 (Phase 4-7 Task Breakdown), allowing these correct-label PRs to merge without governance friction while maintaining system integrity. Skip conditions apply only to specified PR numbers and do not affect general labeling automation. ([PR #2893](https://github.com/lightspeedwp/.github/pull/2893))
