...
- **`lint-and-links` rejecting genuinely-existing CHANGELOG.md issue links** — The `Check Links` step in `meta.yml` was flagging `github.com/lightspeedwp/.github/issues/*` links as 404 (occasionally 502) for issues confirmed via the GitHub API to exist. Two remedies were tried and ruled out: `lycheeverse/lychee-action` already authenticates with `github.token` by default, so an explicit `GITHUB_TOKEN` env changed nothing; retry-with-backoff (`max_retries`/`retry_wait_time` in `lychee.toml`) also had no effect, and the exact same set of ~35 issue numbers (observed range: #31-#991) failed identically on every run — deterministic, not the transient pattern rate limiting would produce. Every issue #1000+ checked (`#1020`, `#1831`, `#1960`) resolves fine. This matches issues created before a data migration/import boundary in the repo's history that the API can still serve but the web renderer 404s on. Excluded 1-3 digit `github.com/lightspeedwp/.github/issues/N` links in `lychee.toml`, preserving link verification for every issue created since. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))
