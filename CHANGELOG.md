...
- **Speckit #008: Branch Cleanup Audit & Refactoring Infrastructure — Phase 1 Complete** — Comprehensive specification and Phase 1 design for auditing and refactoring branch cleanup infrastructure to address 300+ accumulated branches in the `.github` repository. Specification defines 5-phase approach: P1 audit (branch categorisation), P2 safe deletion (verified candidates), P3 refactoring (script harmonisation), with documentation and automated workflow delivery. Phase 1 Design deliverables include: (1) **Feature Specification** (`.github/specs/009-audit-branch-cleanup/spec.md`, 500+ lines) — 5 user stories covering audit generation, safe deletion candidates, discussion categorisation, documentation updates, and scheduled workflow automation; 16 functional requirements; 8 success criteria; 5 edge cases; 12 assumptions covering merge detection, PR analysis, categorisation logic, multi-layer safety verification, and <5 second performance target for 500+ branches; (2) **Phase 0 Research** (`research.md`, 400+ lines) — 8 design decisions: merge detection via `git merge-base`, PR detection via `gh pr list`, multi-gate categorisation logic, dual-format report output (Markdown + JSON), 3-layer safety verification, <5 second performance target, centralised branch naming validation, regex-based exclusion patterns; (3) **Data Model** (`data-model.md`, 250+ lines) — Branch entity (11 attributes + 1 computed status), BranchAuditReport structure, DeletionCandidate entity, categorisation logic with 8-gate decision tree, reason codes (KEEP/DELETE/DISCUSS), branch type taxonomy (30+ types from CLAUDE.md), state transitions, validation rules; (4) **Interface Contracts** (`contracts/audit-report.schema.json`, `contracts/deletion-candidates.schema.json`) — JSON schemas defining audit report structure (timestamp, repository, summary counts, categories with subcategories), deletion candidates format (verification checks, merge status, safety metadata); (5) **Validation Quickstart** (`quickstart.md`, 400+ lines) — 7 runnable scenarios validating audit generation, categorisation logic (5 test cases: protected branch, branch with open PR, merged & stale, naming violation, unmerged & stale), JSON output format, custom exclusion patterns, performance (<5 seconds), data model validation, contract compliance; (6) **Quality Checklist** (`checklists/requirements.md`) — Comprehensive validation confirming all 4 content quality items, 8 requirement completeness items, and 4 feature readiness items passing. **Technical Approach**: Enhance existing `scripts/cleanup-branches.js` with safe categorised audits using Node.js 22, GitHub CLI, and git; dry-run mode as default safe behavior; support custom exclusion patterns and configurable thresholds. **Impact**: Resolves branch accumulation problem (300+ → target <50 active) with clear categorisation logic distinguishing KEEP (protected, active PR, recent), DELETE (merged & stale), and DISCUSS (ambiguous cases) branches. **Related**: Specification address governance audit requirements, branch naming validation enforcement, scheduled workflow automation for ongoing maintenance. See [Branch Cleanup Audit & Refactoring Specification](.github/specs/009-audit-branch-cleanup/spec.md) and [Phase 1 Design Summary](.github/specs/009-audit-branch-cleanup/plan.md). ([PR #3128](https://github.com/lightspeedwp/.github/pull/3128))
