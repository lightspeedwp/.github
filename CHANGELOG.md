...
- **Workflow Reachability Guards** — A test now fails if a composite action has no caller in an active workflow, if a local `uses:` reference does not resolve, or if a workflow-shaped file is committed outside `.github/workflows/`. It reproduces the defect it prevents. ([#3570](https://github.com/lightspeedwp/.github/issues/3570))
...
- **Inert Workflow Test Harness** — Deleted `.github/tests/workflow-harness.yml` and `.github/tests/error-isolation-test.yml`. Both declared workflow triggers but sat in a directory GitHub does not register, so neither had ever run. ([#3570](https://github.com/lightspeedwp/.github/issues/3570))
- **Uncalled Composite Actions** — Deleted `aggregate-tests` and `validate-check`. No active workflow called either, yet their contract tests passed, and #3478's request to remove them was closed without it. ([#3570](https://github.com/lightspeedwp/.github/issues/3570))

### Fixed

- **Composite Action Docs Matched Reality** — Removed the two deleted actions from the reference and replaced the claim that composite actions are tested in `workflow-harness.yml` with how they are actually tested. ([#3570](https://github.com/lightspeedwp/.github/issues/3570))
