...
- **Metrics Aggregator Tolerates Malformed Artifacts** — The branch-validation metrics aggregation now validates each artifact file individually with `jq empty` before aggregating, so one malformed file is skipped with a warning instead of zeroing the whole run and suppressing the metrics PR update. ([#3528](https://github.com/lightspeedwp/.github/issues/3528))
