...
- **Awesome GitHub Site Phase 13: Search command palette** — Site-wide ⌘K search modal (`website/src/components/SearchModal.astro`) added to `AwesomeGithubLayout`. Generates a build-time JSON index from all catalogue items, glossary entries, cookbook recipes, and learn tracks. Client-side fuzzy matching with ranked scoring (exact title → prefix → contains → badge → description). Results grouped by section (Catalogue, Glossary, Cookbook, Learn), highlighted query matches, and keyboard navigation (↑↓ arrow, Enter to open, Escape to close, Tab focus-trap). Fully accessible: `role="dialog"`, `aria-modal`, `aria-activedescendant`, `aria-selected`, focus returns to trigger on dismiss, body scroll-lock. ([#764](https://github.com/lightspeedwp/.github/issues/764))
