...
- **PRD Agent Consolidation — Phase 4 Memory Registry Consolidation** — Completed Phase 4 consolidation tasks (T045-T048) transitioning from `agents/mode-prd.agent.md` to consolidated `agents/prd-agent/copilot/agent.md`. Phase 4 deliverables: (1) **T045 - Registry Consumer Identification** — Scanned for active runtime consumers of `agents/mode-prd.agent.md` and confirmed `workflows/memory/registry/memory-registry.yaml` is documentation-only with no active runtime dependencies; (2) **T046 - Memory Registry Entry Update** — Updated `agent:mode-prd` source_path in memory registry YAML and regenerated `inventory-lock.json` with new agent path; (3) **T047 - Obsolete File Deletion** — Removed `agents/mode-prd.agent.md` as it is now replaced by consolidated agent definition; (4) **T048 - Documentation Link Repair** — Fixed 8 dead links in `docs/AGENT-INDEX.md` pointing to non-existent paths, repointing all references to `../agents/prd-agent/copilot/agent.md`. **Impact**: Single consolidated Copilot agent definition now source of truth; memory registry fully consolidated and consistent; zero dangling references remain. **Related**: Part of PRD Agent Consolidation initiative ([#1248](https://github.com/lightspeedwp/.github/issues/1248)), Phase 7 memory registry work ([#1899](https://github.com/lightspeedwp/.github/issues/1899)). ([PR #2867](https://github.com/lightspeedwp/.github/pull/2867))

- **PRD Agent Consolidation — Phases 4-7 Comprehensive Task Breakdown** — Generated 68-task comprehensive breakdown for PRD Agent Consolidation Phases 4-7 with full specification-to-implementation traceability. Deliverables: (1) **Specification Foundation** — Updated `.github/specs/001-prd-agent-consolidation/spec.md` documenting all Phase 4-7 scope with user stories (US4–US7), acceptance criteria, success gates; (2) **Implementation Plan** — Created `.github/specs/001-prd-agent-consolidation/plan.md` with executable sequential merge process, stacked PR structure with phase gates at Weeks 2, 5, 8, 12; (3) **Task Decomposition** — Generated `.github/specs/001-prd-agent-consolidation/tasks.md` with 68 actionable tasks (T001-T068) organized by phase and stacked PR structure, including prompt enhancement (T001-T020), memory updates (T021-T035), provider testing (T036-T050), adoption (T051-T060), archive/sync operations (T061-T068); each task includes user story traceability, clear acceptance criteria, and file path references; (4) **Phase Gates** — Explicit merge gates placed at critical validation points ensuring each phase completes fully before proceeding. **Task Organization**: Phase 4 (15 tasks, Week 1-2), Phase 5 (15 tasks, Week 2-5), Phase 6 (20 tasks, Week 5-8), Phase 7 (18 tasks, Week 8-12). **Dependency Graph**: Full parallel execution opportunities identified with non-blocking tasks grouped for concurrent delivery. **Impact**: Provides clear implementation roadmap for 4-week consolidation lifecycle; enables parallel execution where dependencies allow; establishes merge gates for validation at each phase boundary. **Related**: PRD Agent Consolidation initiative ([#1248](https://github.com/lightspeedwp/.github/issues/1248)). ([PR #2866](https://github.com/lightspeedwp/.github/pull/2866))

- **CodeRabbit Configuration Optimization — Phase 2 Foundational Work** — Comprehensive configuration improvements enhancing code review coverage, consistency, and alignment with branch naming strategy and labeling system. Phase 2 deliverables: (1) **Specification & Planning** — Complete feature specification (FR-001 through FR-013) with 5 user stories (US1–US5, P1–P3 priority), success criteria, and 68-item specification-quality checklist establishing clear review guidance expectations. Implementation plan documenting 30-40 existing instruction block enhancements and 15-20 new blocks for emerging file types across organization repositories; (2) **Foundational Documentation** — Four comprehensive governance reference guides: BRANCH_CONTEXT_MAPPING.md (15+ branch types review focus areas, critical checks, technology-agnostic guidance), BRANCH_TYPE_PRIORITY_MAPPING.md (four-tier priority classification covering ~96% of organizational usage), INSTRUCTION_BLOCK_STRUCTURE.md (canonical structure standard with 3+ focus areas and 3-5 checks per block), TECHNOLOGY_AGNOSTICISM.md (validation checklist ensuring universal principles across WordPress/Node.js/Infrastructure/MCP domains); (3) **Configuration Audit & Analysis** — CODERABBIT_LABELS_ALIGNMENT.md (100% verification of 158 canonical labels, 10 PR templates, 24 issue types confirming all mappings); Path Priority System documentation (4-tier: exact 90-100%, specific 70-89%, type 50-69%, general 1-49%) with 10+ examples in .coderabbit.yml comments; (4) **Defect Remediation** — Fixed 4 major defects: technology-specific language removal (WordPress/JavaScript references → universal principles), configuration path standardization (.github/coderabbit.yml → .coderabbit.yml), specification path alignment (specs/001 → specs/002), branch-type guidance architecture clarification (external human governance reference per Option C, not CodeRabbit automation); (5) **Validation Scenarios** — 11 quickstart validation scenarios with execution procedures and rollback steps covering file-type coverage, path priority ordering, branch-type documentation, accessibility standards, and configuration integrity; (6) **Spec Organization** — Renamed feature folder from specs/001-coderabbit-config-improvements to specs/002-coderabbit-config-improvements establishing second specification structure and standardized paths to .github/specs/. **Impact**: Establishes foundation for Phase 3 (instruction block enhancements) and Phase 4 (new blocks). **Status**: Option C architecture confirmed (branch-type guidance as external governance reference). **Related**: Extends Phase 1 audit work; supports PR template routing, GitHub Actions workflows, and AI agent decision trees organization-wide. See [CodeRabbit Configuration Project](specs/002-coderabbit-config-improvements/) for complete specification, planning, and task documentation.

- **CodeRabbit Path Instructions — Comprehensive File Type Coverage Expansion** — Enhanced CodeRabbit configuration with expanded path instruction coverage from ~8 to ~24 file type categories, providing targeted code review guidance across 95%+ of repository file types. Deliverables: (1) **Python Ecosystem** — Path instructions for `*.py`, `requirements.txt`, `setup.py`, `pyproject.toml` with validation for PEP 8 compliance, docstrings, type hints, imports, security practices, error handling, test coverage, and logging patterns; (2) **Configuration Files** — Review guidance for `*.toml`, `*.yaml`, `*.xml` formats with schema validation and format consistency checks; (3) **Stylesheets** — Instructions for `*.css`, `*.scss`, `*.sass` with best practices for maintainability and performance; (4) **Modern JavaScript** — ES modules (`.mjs`) and CommonJS (`.cjs`) patterns with proper dependency management; (5) **Component Templates** — Guidance for `*.astro`, `*.mdx`, `*.html` files with markup semantics and accessibility checks; (6) **Data Files** — Review patterns for `*.csv`, `*.json` with schema and format validation; (7) **Meta/Config** — Instructions for `.editorconfig`, `.env*`, `.gitignore`, `Makefile` with consistency and security validation. **Configuration Quality**: Valid YAML syntax (python3 yaml parser: ✅), path pattern correctness, no conflicts with existing patterns, alignment with CodeRabbit v2.0 specification. **Impact:** Reviewers now receive targeted guidance for majority of repository file types without performance degradation, improving code review consistency and reducing missed issues across polyglot codebase. **Note:** PR #2891 documents branch naming convention exception (`claude/zen-bohr-aluig7` prefix) with explicit approval for continuity. ([PR #2891](https://github.com/lightspeedwp/.github/pull/2891))
