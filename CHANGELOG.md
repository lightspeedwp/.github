...
- **Specs Directory Structure Compliance — Constitutional Principle III Alignment** — Fixed critical issue where feature specifications ignored the configured `specs_directory`. Root cause: `.specify/scripts/bash/create-new-feature.sh` hardcoded the specs path without checking configuration. Solution: Implemented configuration-driven resolution with a multi-parser fallback strategy. **Result**: Speckit commands use the configured repository-relative directory, with `.github/specs` as the default when `specs_directory` is not configured; feature numbering remains sequential. See [Specs Directory Fix Specification](.github/specs/007-specs-directory-fix/spec.md). ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
...
- **Specs Directory Fix — Script Updates & Documentation** — Updated scripts to resolve and use the configured `specs_directory`, falling back to `.github/specs` only when the setting is absent. Changes: (1) `.specify/scripts/bash/create-new-feature.sh` uses `read_specs_directory()` instead of a hardcoded path; (2) `.specify/scripts/bash/setup-plan.sh` and `setup-tasks.sh` use the same configuration; (3) `.github/CLAUDE.md` documents `.github/specs/` as this repository's configured location; (4) `.specify/feature.json` references the configured feature location. **Backward Compatibility**: Unconfigured projects now default to `.github/specs` instead of the previous root-level `specs/` location; explicitly configured repository-relative paths remain supported. ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
- **Specs Directory Fix — Phase 7 Convergence Validation Tasks** — Appended 8 convergence validation tasks (T037-T044) to track deferred Phase 6 work for the specs-directory-fix feature. Tasks validate: (1) Full integration test with dry-run (T037); (2) All speckit commands resolving correctly (T038); (3) Legacy path references cleanup via grep (T039); (4) Security validation of `read_specs_directory()` function (T040); (5) Migration script rollback verification (T041); (6) CHANGELOG.md documentation completion (T042); (7) Final integration validation with new test feature (T043); (8) Spec.md status documentation (T044). All validation tasks are mapped to functional requirements (FR-001 through FR-009) and success criteria (SC-001 through SC-006); completion remains pending until these tasks are executed. See [Phase 7 Convergence Tasks](.github/specs/007-specs-directory-fix/tasks.md#phase-7-convergence-validation-pr-3360-follow-up). ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
...
- **Specs Directory Fix — Migration Helper & Configuration** — Implemented configuration-driven specs directory resolution with an automatic rollback mechanism for FR-009. Deliverables: (1) `.specify/scripts/bash/migrate-specs.sh` migrates from root-level `specs/` to the fixed `.github/specs/` destination only when that destination is configured, with backup, rollback, dry-run, and verification steps; (2) `read_specs_directory()` in `.specify/scripts/bash/common.sh` resolves the configured value and defaults to `.github/specs` when unset; (3) `.specify/init-options.json` configures this repository's directory; (4) `.github/specs/007-specs-directory-fix/contracts/init-options-schema.md` documents the field. Valid alternative repository-relative paths remain supported by Speckit commands, while this one-time migration helper fails safely when such a custom destination is active. See [Specs Directory Fix Specification](.github/specs/007-specs-directory-fix/spec.md). ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
