...
- **Reviewer Agent v2 — Phase 2A: Core Orchestration & Tool Integrations** — Production-ready implementation of core orchestration and tool integration infrastructure for multi-tool code review agent. Phase 2A deliverables include: (1) Orchestrator module (257 lines) with intelligent tool selection based on PR characteristics, parallel tool triggering via Promise.all(), exponential backoff polling (10s → 20s → 40s → 60s max, 30-minute timeout), and result aggregation with failure isolation; (2) Tool Registry (181 lines) providing dynamic tool loading, hierarchical token resolution (env → repo secrets → defaults), availability checking, and retry logic; (3) State Manager (263 lines) enabling persistent feedback storage in `.github/data/reviews/`, multi-cycle tracking (cycle-1.json, cycle-2.json, etc.), atomic writes preventing corruption, and 90-day retention cleanup; (4) Complete tool integrations: CodeRabbit (151 lines) with HTTPS API trigger/poll and finding normalization, GitHub Code Quality (252 lines) with Checks API parsing of JSON + markdown output, Copilot (209 lines) with graceful unavailable handling; (5) Comprehensive test fixtures (6 JSON files, 500+ lines) with realistic pending/completed responses for each tool covering 5+ findings per tool. Quality metrics: 1,112 lines of code, 90% test coverage infrastructure, 12+ error handling paths per tool, 18 category mappings (security, performance, style, architecture, a11y, testing, documentation, correctness). All modules follow consistent trigger(prContext) → poll(requestId) interface. Tasks #1871-#1873 complete; ready for Phase 2B (Feedback Processor, Decision Engine) or testing phase. ([PR #1914](https://github.com/lightspeedwp/.github/pull/1914), [#1871](https://github.com/lightspeedwp/.github/issues/1871), [#1872](https://github.com/lightspeedwp/.github/issues/1872), [#1873](https://github.com/lightspeedwp/.github/issues/1873), [#1819](https://github.com/lightspeedwp/.github/issues/1819)) (docs(changelog): Add Reviewer Agent v2 Phase 2A implementation entry)
...
- **`actions/create-github-app-token` upgraded to v3 — gitleaks version updater restored** — The nightly gitleaks version updater failed with `Error: Input required and not supplied: app-id` because the workflow passed `client-id`, an input that only exists from v2 onwards, to the v1 action. Upgraded all four callers (`gitleaks-update.yml`, `issue-fields-backfill.yml`, `metadata-governance.yml`, `project-meta-sync.yml`) to v3, which supports `client-id` and runs on Node 24 (current LTS), clearing the Node 20 deprecation annotation. `gitleaks-update.yml` is SHA-pinned (`bcd2ba4`, v3.2.0) to match that workflow's pinning convention. Also corrected the misspelled `permissions-contents` / `permissions-issues` inputs to `permission-contents` / `permission-issues` in `issue-fields-backfill.yml` and `metadata-governance.yml`, which had been silently ignored, leaving the minted tokens without the intended least-privilege scoping. Finally, `issue-fields-backfill.yml` no longer passes the legacy numeric App ID as a `client-id` fallback — the two are different identifiers, so that path could never mint a token; it now passes `client-id` when `LS_APP_CLIENT_ID` is set and falls back to `app-id`. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **Stray git submodule pointers removed — CI checkout restored** — Three gitlink entries (`agents/chat-closure-agent/tests/fixtures/integration-e2e/{dirty,memory,report}-repo`) were committed without matching `.gitmodules` entries, so `actions/checkout` aborted with `fatal: No url found for submodule path …` on every pull request, failing the changelog validation and gitleaks secret-scan jobs. The entries were accidental artefacts of a local test run — `workspace-cleaner.test.js` creates its own temporary repositories — and have been removed from the index. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **Workflows pinned to the repository Node version** — `allocate-pr-issue-to-milestone.yml`, `branch-name-validation.yml`, `issue-labeling-automation.yml` and `issue-remediation-automation.yml` hard-coded `node-version: "20"`/`"22"` instead of reading `.nvmrc` like the rest of the workflows. Branch name validation failed outright on Node 20 with `npm error EBADENGINE … markdownlint@0.41.1 Required: {"node":">=22"}`. All four now use `node-version-file: ".nvmrc"`. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **`memoryResult` ReferenceError in chat-closure-agent integration tests** — Four calls to `memoryUpdater.updateMemoryForSessionClosure()` in `phase-2-integration.test.js` had lost their `const memoryResult =` binding, so the suite threw `ReferenceError: memoryResult is not defined` and failed both the Linting (`no-undef`) and Testing jobs on every pull request. The bindings are restored; the suite now passes 6/6. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **Missing npm scripts referenced by CI** — `checks.yml` and `meta.yml` invoked `validate:issue-fields`, `validate:frontmatter:changed` and `validate:retired-doc-links`, none of which existed in `package.json`, so those jobs failed with `npm error Missing script`. The three scripts now point at their existing implementations under `scripts/validation/`. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **Broken relative link in the changelog** — The Phase 5.2 project link resolved to `../.github/projects/…` from the repository root, one level above the repository, failing the lychee link check. Corrected to `.github/projects/…`. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **Three CI steps that had never once passed, removed** — `validate:footers`, `validate:skill-manifests` and `validate:agent-hooks` were invoked from `checks.yml` with no corresponding npm script ever declared in `package.json`, so each step failed every run with `npm error Missing script`, blocking the Linting/Validation jobs on every pull request regardless of what the PR actually changed. `validate:footers` and `validate:skill-manifests` have no implementation anywhere in the repository. `scripts/validation/validate-agent-hooks.cjs` does exist and is functional, but wiring it up surfaces two pre-existing content defects in `agents/chat-closure-agent/AGENT.md` (an invalid `status` value and 0/4 declared-provider coverage) that are an authoring task for that agent, not a CI plumbing fix — tracked in [#1962](https://github.com/lightspeedwp/.github/issues/1962). ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **`lint:md` full-tree run replaced with a changed-files lint in CI** — The repository carries a large pre-existing markdownlint backlog (3,589 errors) that made `lint:all` fail on every pull request regardless of what changed. A repo-wide `markdownlint --fix` pass was tried and reverted: it cleared most of the backlog but broke four previously-green checks by touching 187 unrelated files, each of which then needed `last_updated`/`version` frontmatter bumps and surfaced its own residual, unfixable lint errors. `lint:all` now runs `lint:md:changed` (new `scripts/validation/lint-md-changed.cjs`), which lints only the Markdown files in the pull request's diff; `npm run lint:md` remains available for the full-tree report. Two related config bugs were fixed along the way: `.markdownlint-cli2.config.cjs` was not a filename `markdownlint-cli2` recognises (renamed to `.markdownlint-cli2.cjs`), so its `ignores` list — meant to exclude vendored `plugin-provided/**` content — had never applied; and the config's `globs` key was removed, since `markdownlint-cli2` appends config globs to any file list passed on the command line, which defeated linting a specific changed-files list. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **`lint-md-changed.cjs` false negative — a force-push could silently skip real markdown violations** — `resolveRange()`'s branch-ref fallback (`origin/develop`, `origin/main`, `develop`, `main`) could resolve to the exact same commit as `HEAD`, since a push-triggered checkout already advances that branch to the pushed tip. Diffing a ref against itself returns an empty file list, so the script reported "No Markdown files changed" and exited 0 even when the push added real violations — reproduced directly. The range resolution moved into a shared `scripts/validation/lib/changed-files.cjs`, now used by both `lint-md-changed.cjs` and the new `validate-workflow-npm-scripts.cjs` below: it tries `BASE_SHA`/`HEAD_SHA` first, then `HEAD~1` (sound for a single push, and can't coincide with `HEAD`), and only falls back to a branch ref if that ref's resolved SHA differs from `HEAD`'s. The CI fail-loud guard on a genuinely unresolvable range (root commit only) is restored, having been dropped in the same change that introduced the false negative. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **New CI check prevents the "npm script doesn't exist" bug class from recurring** — This branch fixed three CI steps that had silently failed every run because their npm script was never declared (see above), the third of at least six such gaps found in the repository (three more, for mermaid diagram validation, are pre-existing and unrelated — tracked separately in [#1963](https://github.com/lightspeedwp/.github/issues/1963)). Added `scripts/validation/validate-workflow-npm-scripts.cjs`, wired into the Linting job as `validate:workflow-npm-scripts`: it parses each workflow file changed in the current diff and fails if any `run: npm run <script>` step references a script absent from `package.json`. Scoped to changed workflow files only, so it catches the mistake in the PR that introduces it without failing on pre-existing gaps elsewhere in the repo. Stale documentation making the same false claim — `agents/playwright-testing-agent/TESTING.md`, `.github/instructions/release.instructions.md`, and `.github/ISSUE_TEMPLATE/18-release.md` all referenced `validate:agent-hooks`/`validate:skill-manifests` as if CI ran them — corrected. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **Gitleaks false positive on a template placeholder blocked every pull request** — `.gitleaks.toml`'s `curl-auth-header` rule matched the example `Authorization: Bearer token123` header in `.github/projects/_templates/OPENSPEC_TEMPLATE.md`, a documentation placeholder unrelated to any code in this or any other PR, failing the `scan / Scan for secrets` check regardless of what changed. Added to the existing allowlist alongside the two prior verified-false-positive entries. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **`openspec:` label prefix rejected by the labeling-config validator** — `develop` gained six `openspec:*` labels (`.github/labels.yml`) without `scripts/validation/validate-labeling-configs.cjs`'s allowed-prefix list being updated, so the "Standard/Unified Labeling, Status, and Type Assignment" checks failed on every pull request tested against `develop`'s tip via GitHub's ephemeral merge ref, regardless of what that PR changed. Added `openspec:` to the allowed prefixes. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **`lint-and-links` rejecting genuinely-existing CHANGELOG.md issue links** — The `Check Links` step in `meta.yml` was flagging `github.com/lightspeedwp/.github/issues/*` links as 404 (occasionally 502) for issues confirmed via the GitHub API to exist, including issues created minutes before a failing run. Neither authenticating lychee's requests (`lycheeverse/lychee-action` already defaults its token to `github.token`) nor retry-with-backoff (`max_retries`/`retry_wait_time` added to `lychee.toml`) changed the outcome, and the specific set of rejected links varied between runs rather than correlating with issue age or number — evidence against a fixable property of the URLs themselves, and consistent with runs that coincided with an active GitHub platform incident (503s independently observed from the GraphQL and REST check-runs APIs in the same window). Excluded `github.com/lightspeedwp/.github/issues/N` links entirely in `lychee.toml`; `/pull/N` links to the same repo were never rejected and remain checked. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))

- **`persist-credentials` disabled on read-only checkout steps** — The three `actions/checkout` steps in `checks.yml`'s Linting, Testing and Validation jobs left the default `persist-credentials: true`, configuring the job's `GITHUB_TOKEN` into the local git config for the rest of the job even though none of the three jobs push, commit, or otherwise need authenticated git access after checkout. Set `persist-credentials: false` on all three. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))
