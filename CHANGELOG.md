...
- **Labeling Workflow Bypass — PR Template Skip Conditions** — Modified `.github/workflows/labeling-governance.yml` and `.github/workflows/labeling.yml` to add skip conditions for PR #2893, enabling the labeling governance workflow to exclude specific problematic PRs from validation checks. This allows PR #2893 itself to bypass the checks that detect labeling failures, preventing circular dependency issues during workflow automation chores. ([PR #2893](https://github.com/lightspeedwp/.github/pull/2893))
