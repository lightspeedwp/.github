...
- **GitHub Check Run Support for Changelog Validation** — Implemented GitHub Checks API integration to provide proper check run creation with detailed violation annotations visible in PR Checks tab, closing the FR-2 gap identified in the Changelog Quality Audit (Spec 003). Deliverables: (1) **CheckRunReporter class** (`lib/check-run-reporter.js`, 89 lines) supporting full GitHub Checks API with conclusion determination (success/failure/neutral), violation-to-annotation mapping with severity levels, and 50-annotation limit compliance per GitHub API spec; (2) **CLI tool** (`bin/report-check-run.js`, 104 lines) for invoking CheckRunReporter from workflow with yargs argument parsing and verbose logging; (3) **Integration test suite** (`test/integration/test-check-run-annotations.js`, 285 lines) using Vitest with mocked GitHub API client validating check run creation, annotation structure, conclusion logic, context handling, and API compliance; (4) **Workflow integration** (`.github/workflows/changelog-validation.yml`) with proper check run status reporting for both validation runs and skipped validations; (5) **CI fix** ensuring check runs are always created with state-appropriate conclusions, never conditionally skipped. Impact: Users now see validation results as first-class GitHub checks with actionable per-entry violation details, moving from comment-only reporting to integrated GitHub Checks experience. ([PR #3376](https://github.com/lightspeedwp/.github/pull/3376))
