...
- **Multi-provider WordPress Config Agent (Phase 2)** — Standardised `agents/wp-config-agent/` to the Phase 2 multi-provider pattern with real, substantial content: an eight-phase provider-agnostic `shared/core-prompt.md` (site assessment → performance analysis → security assessment → plugin & theme audit → backup & disaster recovery → compliance & accessibility → maintenance & operations → implementation roadmap), Claude tools (`claude/agent.md`, `claude/tools.json`, 8 tools with input schemas), Copilot skills (`copilot/agent.md`, `copilot/skills.yaml`, 8 skills with GitHub integration), and OpenAI functions (`openai/agent.md`, `openai/tools.json`, 8 functions). Includes security guardrails (no credentials, backup-first, staging testing, least privilege). Covers WordPress configuration, performance optimisation, security hardening, plugin/theme management, backup strategy, and maintenance operations. ([PR #1142](https://github.com/lightspeedwp/.github/pull/1142); [#1102](https://github.com/lightspeedwp/.github/issues/1102))
...
- **GitHub Projects creation system — planning & infrastructure** — Comprehensive planning for bulk project creation via CSV fixtures with template-driven field initialization. Planning documents cover field mapping strategy, CSV schema, architecture (new agent + workflow), implementation roadmap (4 phases), and two distinct templates (Client Delivery + Product Development). Created parent Epic #1154 and two child Stories (#1155, #1156). Foundation for Phase 1 implementation. ([PR #1150](https://github.com/lightspeedwp/.github/pull/1150))

- **Organisation-level issue-field writer infrastructure (MVP)** — Scaffolding for GraphQL-based writer to populate organisation-level custom fields (Domain, Delivery Track, Team, Risk, Customer Impact, Technical Impact) on issues and PRs. Currently awaiting org admin field ID configuration; ready for Phase 2 implementation once config file created. Updated `docs/ISSUE_FIELDS.md` with setup guide and GraphQL discovery query for org admins. ([PR #1151](https://github.com/lightspeedwp/.github/pull/1151), relates to #1145)

- **KWV project skills contributed to `skills/design-md-agent/`** — Fleshed out 8 previously-stubbed Figma→`theme.json` skills with the full working instructions developed on the KWV build — `figma-themejson-palette`, `figma-themejson-spacing`, `figma-themejson-typography`, `figma-themejson-radius`, `figma-themejson-shadow`, `figma-themejson-style-variations`, `theme-color-token-enforcer`, and `themejson-extractor-orchestrator` (existing frontmatter and `agents/openai.yaml` interface configs preserved; only the empty bodies were filled in). Added 11 new WordPress block-theme skills from the same project — `block-theme-audit`, `pattern-extractor`, `theme-orphaned-refs`, `themejson-completion`, `agency-scope-change-control`, `wp-agency-project-bootstrap`, `wp-blockstyle-css-field`, `wp-db-override-reconciliation`, `wp-mcp-wpcli-ops`, `wp-pattern-runtime-pitfalls`, and `wp-thirdparty-markup-styling` — each with a `SKILL.md` (frontmatter + LightSpeedWP footer), an `agents/openai.yaml`, and supporting `references/`/`scripts/` where applicable. ([PR #1149](https://github.com/lightspeedwp/.github/pull/1149))
...
- **Project sync workflow: hardened credential error handling** — Preflight check now fails loudly when GitHub App credentials are missing but expected (LS_PROJECT_URL set without LS_APP_ID or LS_APP_PRIVATE_KEY), instead of silently skipping. Distinguishes intentional disable (LS_PROJECT_URL unset) from misconfiguration (credentials missing). Added actionable error message and GitHub App setup guide in `docs/AUTOMATION.md` for org admins. ([PR #1150](https://github.com/lightspeedwp/.github/pull/1150), fixes #1146)

### Fixed

- **Milestone capacity: type exclusion filtering not implemented** — The capacity check configuration specified `exclude_types: [chore, task, documentation]` but the code didn't filter by type when counting issues, causing false positives (chores/tasks counted toward the 50/100 thresholds despite being configured to exclude). Implemented type-based filtering in `getMilestoneStats()` to properly exclude configured issue types. Added 24 unit tests covering milestone allocation logic and capacity exclusion filtering, and documented the allocation strategy for contributors. ([PR #1132](https://github.com/lightspeedwp/.github/pull/1132))

- **Validation tool robustness: footer truncation and mermaid workflow** — Fixed critical data loss bug in `validate-footers.js` where the `replaceFooterTail()` function truncated entire file bodies by using the wrong `---` separator reference (17 files destroyed in PR #1108, now fixed). Improved `.github/workflows/validate-mermaid-pr.yml` error handling for edge cases (git diff failures, empty base refs, no markdown changes) to reduce false CI failures. ([PR #1123](https://github.com/lightspeedwp/.github/pull/1123), [#1118](https://github.com/lightspeedwp/.github/issues/1118), [#1119](https://github.com/lightspeedwp/.github/issues/1119))

- **Meta Agent workflow: missing dependency install and direct push to a protected branch** — The `apply-meta` job ran the meta agent with no `npm ci` step, so `js-yaml` and other dependencies weren't guaranteed to be present, and it failed for real on a `develop` push with `ERR_MODULE_NOT_FOUND`. Separately, `apply-meta` and the former `metrics-update` job pushed straight to `develop`, which is structurally rejected because `develop`'s branch protection requires the `Validate PR Template` check, and that check only runs on `pull_request` events, never on `push`. Added the missing `npm ci` step, merged the two jobs into one, and routed the resulting content/metrics commit through an auto-merged pull request instead of a direct push. ([PR #1073](https://github.com/lightspeedwp/.github/pull/1073), [#1072](https://github.com/lightspeedwp/.github/issues/1072))
...
- **Mergify config: invalid `speculative_checks` field** — `queue_rules[0]` in `.github/mergify.yml` set `speculative_checks: 1`, a field Mergify's schema no longer accepts. This made Mergify's own status checks ("Summary", "Mergify Merge Queue") permanently fail with a config error on every PR, which was enough to block merging into `develop` even when both actually-required checks passed. Removed the unsupported field. ([PR #1077](https://github.com/lightspeedwp/.github/pull/1077), [#1076](https://github.com/lightspeedwp/.github/issues/1076))
