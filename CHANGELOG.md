...
- **Branch Naming Strategy — Alignment with Constitution Principle V** — Updated all branch naming strategy artifacts to enforce 38 authorized types (per Constitution Principle V) instead of 24. Updated documentation (docs/BRANCHING_STRATEGY.md), validation library (lib/validate-branch-name.js), configuration (`.github/branch-types.yml`), and validation scripts to include complete type definitions: feat, fix, hotfix, release, refactor, chore, task, doc, docs, test, perf, ci, build, deps, security, design, a11y, ux, i18n, ops, proto, ds, api, schema, telemetry, content, seo, config, migrate, qa, uat, audit, codex, revert, research, aiops, automation, epic. Fixed missing `release` type in branch validation regex. All governance files now fully aligned with Constitution constraints. ([PR #3353](https://github.com/lightspeedwp/.github/pull/3353))
