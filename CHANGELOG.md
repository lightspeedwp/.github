...
- **`lint-and-links` rejecting genuinely-existing CHANGELOG.md issue links** — The `Check Links` step in `meta.yml` was flagging `github.com/lightspeedwp/.github/issues/*` links as 404 (and occasionally 502) for issues confirmed via the GitHub API to exist. `lycheeverse/lychee-action` already authenticates with `github.token` by default (an explicit `GITHUB_TOKEN` env was added for clarity but wasn't the fix — the token was already present), so this wasn't a rate-limiting-by-lack-of-auth problem; the mixed 404/502 pattern across ~30 issue links checked in a single fast burst is consistent with GitHub's own anti-burst protection on its web frontend, not real broken links. Added `max_retries`/`retry_wait_time` to `lychee.toml` so a rejected request gets exponential-backoff retries instead of failing the run outright. ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))
