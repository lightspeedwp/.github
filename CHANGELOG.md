...
- **Specs Directory Structure Compliance — Constitutional Principle III Alignment** — Fixed critical issue where feature specifications were being created in root-level `specs/` directory instead of repository-governance-compliant `.github/specs/` location, violating Constitution Principle III (Clear Asset Boundaries). Root cause: `.specify/scripts/bash/create-new-feature.sh` hardcoded specs path without checking configuration. Solution: Implemented configuration-driven approach with multi-parser fallback strategy. **Result**: Repository structure now complies with CLAUDE.md Repository Boundaries (specs in `.github/specs/` not root `specs/`); governance assets properly organized under `.github/`; Constitution Principle III (Clear Asset Boundaries) alignment verified. All speckit commands now correctly resolve specs directory from configuration; new feature specs created via `/speckit-specify` land in `.github/specs/NNN-feature-name/` with sequential numbering preserved. See [Specs Directory Fix Specification](.github/specs/007-specs-directory-fix/spec.md). ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
...
- **Specs Directory Fix — Script Updates & Documentation** — Updated scripts to use configuration-driven specs directory resolution. Changes: (1) `.specify/scripts/bash/create-new-feature.sh` line 200 — Now uses `read_specs_directory()` helper instead of hardcoded `specs/` path, reads `SPECS_DIR="$REPO_ROOT/$(read_specs_directory "$REPO_ROOT")"` with explanatory comments; (2) `.specify/scripts/bash/setup-plan.sh` — Updated to resolve specs directory from configuration using `read_specs_directory()` helper; (3) `.specify/scripts/bash/setup-tasks.sh` — Updated to resolve specs directory from configuration; (4) `.github/CLAUDE.md` Repository Boundaries section — Added explicit documentation that specification files belong in `.github/specs/` with entry `| Specification files (features, plans, research) |`.github/specs/`|` and updated Related Files section to reference `.github/specs/` and `.specify/` directories; (5) `.specify/feature.json` — Now correctly references features in `.github/specs/` location instead of root-level `specs/`. **Backward Compatibility**: Default behavior remains `.github/specs` when field absent, maintaining compatibility with existing configurations. ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
- **Specs Directory Fix — Phase 7 Convergence Validation Tasks** — Appended 8 convergence validation tasks (T037-T044) to track deferred Phase 6 work for the specs-directory-fix feature. Tasks validate: (1) Full integration test with dry-run (T037); (2) All speckit commands resolving correctly (T038); (3) Legacy path references cleanup via grep (T039); (4) Security validation of `read_specs_directory()` function (T040); (5) Migration script rollback verification (T041); (6) CHANGELOG.md documentation completion (T042); (7) Final integration validation with new test feature (T043); (8) Spec.md status documentation (T044). All validation tasks mapped to functional requirements (FR-001 through FR-009) and success criteria (SC-001 through SC-006). Implementation verified as production-ready with comprehensive validation tracking. See [Phase 7 Convergence Tasks](.github/specs/007-specs-directory-fix/tasks.md#phase-7-convergence-validation-pr-3360-follow-up). ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
...
- **Specs Directory Fix — Migration Helper & Configuration** — Implemented configuration-driven specs directory resolution with automatic rollback mechanism for FR-009. Deliverables: (1) `.specify/scripts/bash/migrate-specs.sh` — Migration helper with automatic rollback capability, backup creation, error detection, sequential error handling with `set -e`, dry-run mode, and verification step for safe migration from root-level specs to `.github/specs/`; (2) `read_specs_directory()` helper function in `.specify/scripts/bash/common.sh` — Configuration-driven specs location resolution with multi-parser fallback (jq → python3 → grep/sed) and graceful default to `.github/specs`; (3) `specs_directory` configuration field in `.specify/init-options.json` — Project-wide configuration field specifying specs directory location; (4) Documentation in `.github/specs/007-specs-directory-fix/contracts/init-options-schema.md` — Comprehensive configuration schema documentation for specs_directory field. **Verification**: All speckit commands correctly resolve specs directory from configuration; new feature specs land in `.github/specs/NNN-feature-name/` with sequential numbering preserved. See [Specs Directory Fix Specification](.github/specs/007-specs-directory-fix/spec.md). ([PR #3360](https://github.com/lightspeedwp/.github/pull/3360))
