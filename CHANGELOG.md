...
- **`lint-and-links` failing on genuinely-existing CHANGELOG.md issue links** — The `Check Links` step in `meta.yml` runs `lycheeverse/lychee-action` without a `GITHUB_TOKEN`, so its unauthenticated requests to `github.com/lightspeedwp/.github/issues/*` were rejected as 404s — confirmed via the GitHub API that the flagged issues genuinely exist — a well-documented `lychee` failure mode once a Markdown file references more than a handful of GitHub issues/PRs in one CI run. Added `GITHUB_TOKEN` to the step's `env`, per `lychee --github-token`'s documented purpose ("to avoid rate limiting"). ([PR #1959](https://github.com/lightspeedwp/.github/pull/1959), [#1960](https://github.com/lightspeedwp/.github/issues/1960))
