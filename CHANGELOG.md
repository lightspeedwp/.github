...
### Added
- **PR Automation Scripts — Label Management & Review Tracking** — Two lightweight automation scripts for managing PR status labels based on review progress and changelog requirements. Deliverables include: (1) `update-pr-changelog-review.js` (337 LOC) — comprehensive review status tracking with PR review data fetching, status determination (awaiting-review, approved, changes-requested, draft, merged), automatic label updates, dry-run/interactive/auto modes, rate limiting (2s delay every 10 PRs), and error recovery; (2) `update-pr-labels-simple.js` (168 LOC) — lightweight label-only updater for quick synchronization with minimal API calls, suitable for CI/CD pipelines with limited quota; (3) Test suites for both scripts (250+ LOC total) covering status determination, label management, argument parsing, and error handling; (4) Integration with meta:needs-changelog workflow to keep PR status labels synchronized with actual review progress. Both scripts support dry-run mode for safe preview and auto mode for batch updates. Used to manage PRs awaiting review, under review, needing updates, and ready for changelog. ([PR #2015](https://github.com/lightspeedwp/.github/pull/2015))
