...
function validateIdSequence(items) {
  ...
  items.forEach((item, index) => {
    ...
    const num = extractNumberFromId(item.id);
    if (num !== expectedNum) {
      errors.push(`Item ${index}: Gap in sequence (expected CHK-${String(expectedNum).padStart(3, '0')}, got CHK-${String(num).padStart(3, '0')})`);
    }
    expectedNum = num + 1;
    ...
    if (item.dimension) {
      ...
      if (idDimension !== expectedDimension) {
        warnings.push(`Item ${index}: Dimension mismatch (ID has '${idDimension}', item has '${expectedDimension}')`);
      }
    }
  });
