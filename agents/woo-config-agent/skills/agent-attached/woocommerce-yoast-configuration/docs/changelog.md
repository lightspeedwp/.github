...
## 2026-07-04 - WooCommerce Yoast Configuration fork

- Renamed the package to `woocommerce-yoast-configuration` for WooCommerce Configuration Agent use.
- Preserved WooCommerce SEO, product schema, product archive, variation, offer, review, stock, shipping, return, category, tag, facet, filtered URL, ecommerce migration and handoff workflows.
- Removed the remaining non-ecommerce sector-specific wording from the generic WordPress site intake example.
- Updated routing references, agent metadata and validation expectations to use the new skill name while keeping live Yoast admin edits routed to `woocommerce-yoast-auditor`.
