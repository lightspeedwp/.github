...
def main(root="."):
    ...
    for p in root.rglob("*"):
        if p.is_file() and p.suffix.lower() in {".md", ".json", ".yaml", ".yml"}:
            try:
                text_corpus.append(p.read_text(encoding="utf-8", errors="ignore").lower())
            except Exception:
                continue
    joined = "\n".join(text_corpus)
    tour_hits = [marker for marker in forbidden_tour_markers if marker in joined]
    if tour_hits:
        return fail({"forbidden_tour_markers": tour_hits})

    generic_routes = ["`yoast-" + "auditor`", "skills://yoast-" + "auditor", "`yoast-" + "configuration`", "skills://yoast-" + "configuration"]
    route_corpus_strict = "\n".join([
        skill_text,
        routing_text,
        future_text,
        (root / "references/related-skills-routing.md").read_text(encoding="utf-8"),
        (root / "tests/related-skills-routing-scenario-tests.md").read_text(encoding="utf-8"),
    ])
    route_hits = [marker for marker in generic_routes if marker in route_corpus_strict]
    if route_hits:
        return fail({"forbidden_generic_skill_routes": route_hits})
