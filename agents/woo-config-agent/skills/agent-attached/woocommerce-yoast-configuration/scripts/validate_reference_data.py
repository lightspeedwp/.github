...
def main(root="."):
    ...
    if frontmatter.get("name") != "woocommerce-yoast-configuration":
        print({"invalid_name": frontmatter.get("name")})
        return 1
    desc = frontmatter.get("description", "")
    if len(desc) < 200 or "woocommerce" not in desc or "schema" not in desc or "woocommerce core" not in desc:
