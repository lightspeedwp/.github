...
- Refocused triggering, routing examples, intake prompts and output contracts on WooCommerce stores only.
- Removed broad non-WooCommerce audit profiles that are not needed for the WooCommerce Configuration Agent.
- Confirmed no non-WooCommerce vertical files were present in the source skill folder during conversion.
