...
- [ ] WooCommerce defaults are scoped to enquiry/product flows unless explicitly expanded.
...
- [ ] The skill has QA scenarios for contact, quote, WooCommerce enquiry, payment, and user registration forms.
...
- [ ] Test prompts include at least 100 numbered cases and cover Post Fields, APC feed review, missing CPTs, anonymous UGC, media-library mapping, taxonomy creation, business listings, editor handoff, post editing, and WooCommerce product route-away.
- [ ] Local validators require the new files and pass all bundled examples.
...
- [ ] Full audit reports route to `woocommerce-gravity-forms-auditor`.
- [ ] Formal findings registers, readiness scorecards, client-safe audit summaries, retainer audit reports, and separate audit retest reports route to `woocommerce-gravity-forms-auditor`.
...
- [ ] Existing WordPress and WooCommerce profiles still work.
...
- [ ] Tests cover routing full audits to `woocommerce-gravity-forms-auditor` and remediation/troubleshooting staying in `woocommerce-gravity-forms-configuration`.
