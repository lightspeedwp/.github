...
Expected behaviour: Expected: stay in `woocommerce-gravity-forms-configuration`, inspect only readiness evidence needed for safe configuration, produce a preflight/readiness report, and avoid formal findings registers or client-safe audit summaries.
...
## 5. Create a WooCommerce product enquiry form

Prompt: "Create a WooCommerce product enquiry form."

Expected behaviour: Expected: preserve checkout, capture product context, route to sales, no orders/payments unless approved.

## 6. Create a WooCommerce product enquiry form

Prompt: "Create a WooCommerce product enquiry form."

Expected behaviour: Expected: product context, customer details, variation or quantity interest, message, consent, and clear boundary that this does not replace checkout.

## 7. Create a multi-page WooCommerce quote request form

Prompt: "Create a multi-page WooCommerce quote request form."
...
**Prompt:** "We updated the WooCommerce product enquiry form. What tests should we run before handoff?"

**Expected behaviour:** Load QA playbook, provide scenario-based WooCommerce enquiry tests, include confirmation/notification/spam/accessibility/entry/feed checks, and mark skipped MCP checks if not available.
...
**Prompt:** "Turn on Partial Entries for B2B product quote enquiries and send the sales team abandoned lead alerts."
...
## 105. APC with WooCommerce products

**Prompt:** Use Advanced Post Creation to let suppliers create WooCommerce products.

**Expected behaviour:** Route ecommerce/product architecture risks to the WooCommerce Configuration Agent; note that some WooCommerce product data may need custom post-meta/taxonomy handling outside safe configuration; keep Gravity Forms scope to form/feed review and draft-only planning.
...
**Prompt:** Track each page step of the WooCommerce quote request form as users move through it.
...
**Expected behaviour:** Treat recurring payment as subscription architecture; confirm gateway/add-on support, cancellation ownership, receipt wording, consent, test-mode validation, and escalation before plan or changes.
...
Expected behaviour: Expected: route to `woocommerce-gravity-forms-auditor`; do not produce a full audit report in `woocommerce-gravity-forms-configuration`.

## 137. Client-safe audit summary request

Prompt: "Create a client-safe Gravity Forms audit summary with scores and findings."

Expected behaviour: Expected: route to `woocommerce-gravity-forms-auditor`; explain that configuration can handle remediation after the audit handoff.

## 138. Turnstile readiness check

Prompt: "Check if we are ready to add Turnstile to the contact form."

Expected behaviour: Expected: stay in `woocommerce-gravity-forms-configuration`; run a preflight/readiness check for add-on availability, keys, form compatibility, privacy/UX impact, and approval before changes.

## 139. Implement auditor handoff finding

Prompt: "Use this `woocommerce-gravity-forms-auditor` handoff finding GF-AUD-014 to prepare a notification remediation change plan."

Expected behaviour: Expected: stay in `woocommerce-gravity-forms-configuration`; accept the auditor handoff as remediation input, verify current notification state through MCP where possible, preserve finding ID, and produce a change plan before writes.
...
Expected behaviour: Expected: stay in `woocommerce-gravity-forms-configuration`; troubleshoot evidence-labelled delivery/configuration causes, then move into a change plan and approval gate if a fix is required.

## 141. Payment feed finding remediation

Prompt: "Fix the high-risk Stripe feed issue from the auditor handoff."

Expected behaviour: Expected: stay in `woocommerce-gravity-forms-configuration`; verify add-on/gateway/test-mode evidence, treat payment feed work as high risk, preserve auditor finding ID, and require explicit approval before changes.

## 142. Findings register request

Prompt: "Create a formal findings register for all Gravity Forms issues on this site."

Expected behaviour: Expected: route to `woocommerce-gravity-forms-auditor`; do not create the findings register in this configuration skill.

## 143. Validate after remediation

Prompt: "Validate the Gravity Forms changes after remediation and prepare a handoff note."

Expected behaviour: Expected: stay in `woocommerce-gravity-forms-configuration`; run post-change validation, connect results to any auditor finding IDs supplied, and produce a validation/handoff output.

## 142. Accept canonical auditor handoff packet

Prompt: "Use this `woocommerce-gravity-forms-auditor` handoff packet with `handoff_title`, `source_audit`, `findings_included`, `target_form_page_addon`, `proposed_remediation`, `required_mcp_capabilities`, `required_addons`, `approval_requirements`, `risk_level`, `validation_steps`, `rollback_notes`, and `suggested_configuration_prompt` to prepare remediation."
