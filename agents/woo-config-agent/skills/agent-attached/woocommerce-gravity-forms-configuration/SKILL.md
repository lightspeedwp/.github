---
name: woocommerce-gravity-forms-configuration
description: configure, validate, troubleshoot, and document gravity forms setups for wordpress websites running the woocommerce core plugin and woocommerce extension plugins. use for woocommerce gravity forms preflight/readiness checks, product enquiry forms, quote request forms, product option forms, deposit/payment forms, account/onboarding flows, notifications, confirmations, conditional logic, spam protection, accessibility, security, add-ons, payments, approved implementation changes, post-change validation, troubleshooting, and handoff notes. for full read-only woocommerce gravity forms audit reports, findings registers, scorecards, and client-safe audit summaries, route to woocommerce-gravity-forms-auditor.
---
# WooCommerce Gravity Forms Configuration

## Operating principle

Make WooCommerce Gravity Forms work predictable, source-aware, API-first, and safe. Treat the active WordPress MCP app as a capability surface, not proof that every Gravity Forms or WordPress operation is available. Discover capabilities, inspect current settings, draft a reversible change plan, get approval for consequential write operations, and validate after change.
...
Use this skill only for WordPress websites where WooCommerce core is installed and active, and the Gravity Forms work supports a WooCommerce or WooCommerce-extension flow. In-scope work includes lightweight fast audit snapshots before configuration, lightweight preflight/readiness checks before configuration, new form planning, existing form remediation, notification/confirmation setup, feed/add-on setup, spam/security configuration, accessibility-aware configuration fixes, WooCommerce product enquiry forms, quote request forms, product option forms, deposit/payment forms, account/onboarding flows, payment forms tied to approved WooCommerce decisions, product/customer support forms, post-purchase surveys, analytics/tracking configuration for WooCommerce-related forms, approved MCP change workflows, post-change validation, troubleshooting, remediation handoffs, and LightSpeed handoff notes.

Use this skill to implement or plan remediation from a `woocommerce-gravity-forms-auditor` handoff after verifying current site state. Do not use it as the primary owner for full audit reports, findings registers, scorecards, client-safe audit summaries, or retainer health-check reports.

## When not to use this skill

Do not use it for non-WooCommerce Gravity Forms work, general WordPress configuration unrelated to WooCommerce Gravity Forms, full WooCommerce checkout architecture, custom plugin development, legal privacy-policy drafting, whole-site accessibility or performance audits, visual Figma design work, page copywriting beyond form microcopy, launch QA beyond Gravity Forms-specific checks, or payment/subscription/tax architecture that is not explicitly scoped as an approved WooCommerce Gravity Forms flow.

Do not use this skill for full Gravity Forms audit reports, formal findings registers, readiness scorecards, client-safe audit summaries, retainer audit reports, or retest reports. Use `woocommerce-gravity-forms-auditor` for those, then return to this skill for approved remediation planning and implementation.

## Auditor/configuration boundary

Use `woocommerce-gravity-forms-auditor` when the requested outcome is a formal read-only audit, scorecard, inspection report, comparison report, findings register, client-safe audit summary, retainer health check, or separate retest report.
...
When receiving a `woocommerce-gravity-forms-auditor` handoff, load `references/auditor-configuration-contract.md`.
...
- If the user needs scoring, a formal findings register, client-safe reporting, retainer audit, or broad multi-form review, route to `woocommerce-gravity-forms-auditor`.
...
Choose one primary output and keep it evidence-labelled. If the requested output is a full audit report, findings register, readiness scorecard, client-safe audit summary, retainer health check, or separate retest report, route to `woocommerce-gravity-forms-auditor` instead of producing it here.
