...
This is the default validation entry point after meaningful changes to docs, schemas, scripts, instructions, saved recurring prompts, starter prompts, short description, file structure, or memory guidance.

The runner is designed to fail early on the highest-value drift first, so maintainers should review failures in the printed order.

## Current routed local skill set to keep aligned

When validation wording, source snapshots, scenario files, or saved recurring prompts mention the current routed workflows, keep them aligned with this local skill set:
...
This section summarises the current validator pack and the files each validator is expected to keep aligned.
...
- route-to-schema wording that overstates dedicated schema coverage
...
- source-snapshot wording that drifts from the main instructions or saved prompt library
...
- `prompts/schema-coverage-audit-prompt.md`
- `prompts/readme-refresh-prompt.md`
- `prompts/skills-routing-directory-validation-prompt.md`
- `prompts/skills-routing-directory-repair-prompt.md`
...
- memory guidance or memory-file structure
