...
export const themeRules = {
  ...
  rule_006_theme_version_semantic() {
    return {
      ...
      description:
        "Theme version should follow semantic versioning (MAJOR.MINOR.PATCH)",
  ...
  rule_010_theme_json_version_two() {
    return {
      ...
      description:
        "theme.json should use version 2 or later (version 1 is deprecated)",
      example: '"version": 2',
  ...
  rule_011_theme_json_root_schemas() {
    return {
      ...
      severity: "info",
      pattern:
        /"(version|settings|styles|customTemplates|patterns|templateParts)"/,
      description:
        "theme.json should include key sections: version, settings, styles",
      example: '{\n  "version": 2,\n  "settings": {...},\n  "styles": {...}\n}',
  ...
  rule_012_theme_json_settings_organized() {
    return {
      ...
      severity: "info",
      pattern:
        /"settings":\s*\{[\s\S]*?"(appearance|spacing|typography|border|color|layout)":/,
      description:
        "Theme settings should be organized by category (appearance, spacing, etc.)",
  ...
  rule_014_design_tokens_centralized() {
    return {
      ...
      pattern: /"(--.*)":\s*".*"/,
      description:
        "Design tokens should be defined in theme.json, not scattered in CSS",
  ...
  rule_015_color_palette_defined() {
    return {
      ...
      example:
        '"palette": [{"name": "Primary", "slug": "primary", "color": "#0073aa"}]',
  ...
  rule_016_typography_defined() {
    return {
      ...
      description:
        "theme.json should define typography settings (font sizes, line heights, etc.)",
  ...
  rule_018_responsive_typography_enabled() {
    return {
      ...
      description:
        "theme.json should enable fluid typography for responsive fonts",
  ...
  rule_019_block_spacing_defaults() {
    return {
      ...
      description:
        "theme.json should define default block spacing to prevent layout shifts",
  ...
  rule_020_border_settings_consistent() {
    return {
      ...
      example:
        '"border": {"radius": [{"name": "Small", "slug": "small", "size": "4px"}]}',
  ...
  rule_021_templates_folder_exists() {
    return {
      ...
      pattern: /templates\//,
      description:
        "Theme should have a 'templates/' directory for block templates",
  ...
  rule_022_template_parts_folder_exists() {
    return {
      ...
      pattern: /parts\//,
      description:
        "Theme should have a 'parts/' directory for reusable template parts",
  ...
  rule_024_single_template_exists() {
    return {
      ...
      description:
        "Theme should have templates/single.html for single post display",
  ...
  rule_026_templates_use_block_markup() {
    return {
      ...
      pattern: /<!-- wp:([a-z]+\/)?[a-z-]+(?: |--)/,
      description:
        "Templates should use HTML comments for block markup, not PHP templates",
  ...
  rule_027_theme_json_template_parts() {
    return {
      ...
      description:
        "theme.json should register template parts with area (header, footer, etc.)",
  ...
  rule_028_patterns_folder_organized() {
    return {
      ...
      pattern: /patterns\//,
      description:
        "Block patterns should be in dedicated 'patterns/' directory",
  ...
  rule_030_wp_head_wp_footer_in_template() {
    return {
      ...
      description:
        "Templates must call wp_head() in header and wp_footer() before closing body",
      example:
        '<!-- wp:html\n{"value":"<?php wp_head();?>"} /-->\n... content ...\n<!-- wp:html\n{"value":"<?php wp_footer();?>"} /-->',
  ...
  rule_031_scripts_enqueued_properly() {
    return {
      ...
      description:
        "CSS and JS should be enqueued in functions.php, not hardcoded in templates",
      example:
  ...
  rule_032_theme_support_declared() {
    return {
      ...
      description:
        "functions.php should declare theme support features (post-thumbnails, menus, etc.)",
      example:
  ...
  rule_037_css_uses_custom_properties() {
    return {
      ...
      pattern: /var\(--[a-z0-9-]+\)/,
      description:
        "CSS should use CSS custom properties (variables) for design tokens",
  ...
  rule_038_css_mobile_first() {
    return {
      ...
      description:
        "CSS should use min-width media queries (mobile-first), not max-width",
  ...
  rule_039_no_hardcoded_colors() {
    return {
      ...
      severity: "warning",
      pattern:
        /color:\s*#[0-9a-fA-F]{3,6}|background(-color)?:\s*#[0-9a-fA-F]{3,6}/,
      description:
  ...
  rule_040_css_uses_logical_properties() {
    return {
      ...
      description:
        "CSS should use logical properties (margin-inline, padding-block) for RTL support",
  ...
  rule_041_style_css_minimal() {
    return {
      ...
      pattern: /^[^{]*\{[^}]{100,}/m,
      description:
        "style.css should contain only header comment, main CSS should be in separate files",
      example:
  ...
  rule_043_semantic_html_used() {
    return {
      ...
      description:
        "Templates should use semantic HTML elements (nav, main, section, etc.)",
  ...
  rule_044_aria_labels_present() {
    return {
      ...
      description:
        "Icon-only buttons and interactive elements should have ARIA labels",
  ...
  rule_045_color_contrast_sufficient() {
    return {
      ...
      description:
        "Text should have sufficient contrast (4.5:1 for normal text, 3:1 for large text)",
  ...
  rule_047_fonts_subset_for_performance() {
    return {
      ...
      pattern: /subset=/,
      description:
        "Google Fonts and web fonts should be subset to reduce download size",
      example:
  ...
  rule_052_footer_scripts_prioritized() {
    return {
      ...
      description:
        "Non-critical scripts should be enqueued in footer (in_footer = true)",
      example:
        "wp_enqueue_script('name', 'path', [], false, true);  // true = footer",
