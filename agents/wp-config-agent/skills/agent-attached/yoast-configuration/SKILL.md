---
name: yoast-configuration
description: configure, audit, validate, troubleshoot and document yoast seo setups for wordpress and woocommerce sites, including schema, metadata, sitemaps, canonicals, redirects, breadcrumbs, product schema, ai metadata, multilingual, taxonomy, migration and research-pack workflows. use for planning, reports, evidence interpretation, decision packs, remediation plans and handoffs; route live wordpress admin audits or approved yoast edits to yoast-auditor.
---

# Yoast Configuration

Use this skill to produce agency-ready Yoast SEO configuration plans, audits, QA checklists, troubleshooting notes, research packs, source registers, and developer handoffs for WordPress and WooCommerce sites.

## Operating posture

- Keep the answer practical, evidence-led, UK English, and suitable for a WordPress agency workflow.
- Treat this skill as a configuration and validation workflow, not a generic SEO tutor.
...
- Route to full audit, rendered-output QA, research refresh, or `yoast-auditor` only when the fast pass exposes a material blocker or live admin inspection/editing is needed.
...
- **Planning/configuration/reporting**: keep in `yoast-configuration`.
- **Live WordPress admin inspection or edits**: route to `yoast-auditor` with confirmed evidence, proposed change, approval state, fields affected, risk, QA, and rollback notes.
...
If the request mixes planning and live edits, complete the planning/decision output first, then route the implementation portion to `yoast-auditor`.

If the installed live-auditor skill uses a different slug, replace `yoast-auditor` with that exact installed slug throughout the routing layer.
...
1. Identify the site type: business, local, publisher/blog, ecommerce catalogue, ecommerce transactional, multilingual, migration/rebuild, duplicate-content-heavy, poor-content-structure, or schema-customisation.
