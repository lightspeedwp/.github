...
Use this contract when preparing findings for `gravity-forms-configuration`. The auditor must not apply the changes.
...
- Suggested prompt for `gravity-forms-configuration`: direct prompt with enough context and no hidden assumptions.
