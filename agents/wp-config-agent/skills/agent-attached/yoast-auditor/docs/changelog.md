...
- Added client-safe note wording to page metadata, taxonomy/archive and evidence gap templates so partial reports stay safe if reused in client-facing documents.
...
- Renamed `wordpress-yoast-seo-auditor` to `yoast-auditor`.
- Refocused the skill on audit, report, QA, completed-change validation and developer handoff.
- Removed edit ownership as a default behaviour.
- Added explicit routing boundary with `yoast-configuration`.
