...
Every template with `## Overall Yoast audit score` must also include fallback wording that allows the assistant to replace the score section with `## Score status` when evidence is too narrow to justify scoring.
