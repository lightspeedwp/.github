# Yoast Configuration Skill Concrete Edit Plan
...
Use this plan to edit the attached local `yoast-configuration` skill so it fits the current **WordPress Configuration Agent**.

## Objective

Convert `yoast-configuration` from a mixed **WordPress + WooCommerce** Yoast skill into a **WordPress-only Yoast configuration** skill.
...
- remove WooCommerce-specific:
  ...
  - mandatory QA wording
  - risk/severity wording
...
### Phase 2 — Decide package boundaries
...
### Phase 3 — Rewrite mixed-scope docs and routing references
...
### Phase 4 — Rewrite reference layer
...
- replace WooCommerce-oriented examples with WordPress-only Yoast examples such as:
  ...
  - breadcrumb or schema issues
  - migration redirect QA
  - sitemap/indexation review
...
- all retained WordPress-oriented profiles
