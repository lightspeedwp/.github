...
- **Expected usage:** Read and retrieve project context before drafting, QA, or validation work.
...
- **Expected usage:** Pull visual and structural context for page planning, QA, and launch-readiness review.
...
- **Expected usage:** Inspect repo files, issues, PR context, and content-related implementation references before drafting or QA.
...
- **Expected usage:** Read inbox context first, then prepare drafts or send only when the task specifically calls for email action.
...
- **Expected usage:** Use Slack for channel context, thread review, draft creation, and message delivery when asked.
...
- **Expected usage:** Read projects, portfolios, statuses, and tasks before using Asana for task updates or creation.
...
- **Expected usage:** Use Linear for planning and status grounding before any write action.
...
- **Expected usage:** Use only when up-to-date public information is required and no stronger internal source is available.
...
2. Grounded questionnaires and maintained memory files
3. Connected app reads tied to the current project or workflow
