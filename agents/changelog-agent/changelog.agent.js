...
async function validateEntry(entry = {}, options = {}) {
  ...
  try {
    ...
    if (autoFormat && result.errors.length > 0) {
      ...
      const reformattedErrors = (revalidation.validation?.summary?.issues || []).map(
...
async function validateChangelog(changelogPath, options = {}) {
  ...
  try {
    ...
    if (!/^## \[\d+\.\d+\.\d+\]/m.test(content)) {
      structureErrors.push('No release version sections (## [x.y.z]) found');
    }
...
async function processChangelog(changelogPath, version, date) {
  ...
  try {
    ...
    const updated = parser.convertUnreleasedToRelease(parsed.content, version, date);
...
async function auditRelease(changelogPath, version, options = {}) {
  ...
  const result = {
    ...
    scope: version ? `release:${version}` : `date-range:${fromDate}-${toDate}`,
    version,
    branch,
    total_entries: 0,
    entries: [],
    ...
    for (const section of releaseEntries) {
      ...
      for (const itemText of items) {
        ...
        const entryResult = {
          ...
          status: validation.validation?.complianceStatus || validation.complianceStatus,
        };

        result.entries.push(entryResult);

        // Categorize by status
        const status = validation.validation?.complianceStatus || validation.complianceStatus;
        if (status === 'passing') {
 ...
 * @param {string} changelogPath - Path to changelog file
 * @param {Object} options - { version, includeDetails, metricsDir }
 *   metricsDir defaults to .github/reports/changelog-metrics.
...
async function collectMetricsSnapshot(changelogPath, options = {}) {
  ...
  const { version = null, metricsDir } = options;
...
async function exportMetricsToCSV(metricsDir, options = {}) {
  ...
  try {
    ...
    const csvRows = snapshots.map((s) => {
      ...
      const mostCommonStr = mostCommon ? `${mostCommon.rule_id}: ${mostCommon.count}` : 'N/A';
...
async function exportReleaseNotes(changelogPath, version, options = {}) {
  ...
  try {
        ...
        if (includeLinks) {
          ...
          for (const issue of issues) {
            entry.issue_links.push({
              number: issue,
              url: referenceLinker.buildIssueUrl('lightspeedwp', 'ls-flow', issue),
              valid: true,
            });
          }
    ...
    switch (format.toLowerCase()) {
      case 'html':
        content = releaseNotesGen.generateHTMLReleaseNotes(version, releaseDate, entries);
        break;
      case 'plaintext':
      case 'plain':
      case 'text':
        content = releaseNotesGen.generatePlainTextReleaseNotes(version, releaseDate, entries);
        break;
      case 'markdown':
      case 'md':
      default:
        content = releaseNotesGen.generateMarkdownReleaseNotes(version, releaseDate, entries);
        break;
    }
...
async function queryMetricsTrend(options = {}) {
  ...
  const { days = 30, metricsDir = '.github/reports/changelog-metrics' } = options;
    ...
    if (fs.existsSync(metricsDir)) {
      ...
      for (const file of files) {
        ...
        if (fileDate >= startDate && fileDate <= now) {
          const snapshotData = metricsBuilder.loadMetricsSnapshot(dateStr, metricsDir);
          if (snapshotData) {
