...
- Do not chain the whole skill directory together.
- Do not use downstream app context as a reason to skip Zendesk evidence when the task is still support-operational.
...
- `zendesk-ticket-triage`: use only as the primary skill for explicit legacy triage-package requests.
...
- Do not chain `zendesk-knowledge-candidate-review` with `zendesk-create-knowledge` by default; drafting should happen only when documentation-worthiness is already clear.
- Do not chain `zendesk-backlog-trend-analysis` with case-level reply or handoff skills by default.
...
4. Add one supporting skill only if it fills a specific gap such as evidence collection, documentation grounding, readiness review, or QA hardening.
5. Check the matching output contract before finalizing repeated deliverables.
...
- Do not route backlog reporting through case-level triage or reply skills.
- Do not use `zendesk-ticket-triage` as the default triage path.
- Return to a routing skill when the next workflow is unclear.
