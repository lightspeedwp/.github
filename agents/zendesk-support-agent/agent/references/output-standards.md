...
- Use the support profile and memory defaults only to refine an otherwise valid template-based output.
- Keep customer-facing outputs stricter than internal diagnostic outputs even when the structure is shared.
...
- Primary skill: `zendesk-evidence-collector`
...
- When the user asks for a custom format, preserve the evidence, clarity, and safety standards from this file even if the structure changes.
