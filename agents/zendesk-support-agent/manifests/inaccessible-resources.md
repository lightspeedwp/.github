...
| Resource name | Skill or agent component | Path or URI | Access attempted | Result | Reason supplied by environment | Metadata captured |
| --- | --- | --- | --- | --- | --- | --- |
| LightSpeed Zendesk connector tools | connector capability | LightSpeed Zendesk app tools visible in runtime | filesystem export | metadata only | Connector implementation and credentials are platform-managed; no source files exposed to filesystem. | yes |
| Google Drive connector tools | connector capability | Google Drive app tools visible in runtime | filesystem export | metadata only | Connector implementation and credentials are platform-managed; no source files exposed to filesystem. | yes |
