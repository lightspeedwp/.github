...
When to read: visual cleanup, formatting, overflow, spacing, alignment, crowding, crop, or deck-wide visual consistency.

## Workflow

1. Confirm the deck and requested scope.
2. Build a slide checklist for the scope.
3. Work one slide at a time unless the same structural defect repeats across slides.
4. For each slide, read the live slide structure and follow `reference-thumbnail-visual-verification.md`.
5. Diagnose the 2-4 concrete visible issues that matter most.
6. Apply one focused batch using live object IDs.
7. Re-read the slide and inspect a fresh thumbnail.
8. Patch again only while concrete visual defects remain.
