...
class ReleaseGates {
  constructor(options = {}) {
    this.config = {
      agenticScoreThreshold: options.agenticScoreThreshold || 0.8,
  ...
  saveAuditLog() {
    ...
    const log = {
      ...
      outcome: Object.values(this.results).every((g) => g.passed)
        ? "SUCCESS"
        : "FAILED",
    };
  ...
  gate2AgenticScore() {
    ...
    try {
      ...
      const entries = (
        changelog.match(
          /^###\s+(Added|Changed|Fixed|Deprecated|Removed|Security)/gm,
        ) || []
      ).length;
      if (entries === 0) {
        score -= 0.1;
      ...
      if (score >= this.config.agenticScoreThreshold) {
        ...
        this.log(
          `✅ GATE 2 PASSED: Agentic score ${(score * 100).toFixed(0)}%`,
        );
      } else {
        this.results.gate2_agentic.passed = false;
        details.push(
          `❌ Score ${(score * 100).toFixed(0)}% < threshold ${(this.config.agenticScoreThreshold * 100).toFixed(0)}%`,
        );
      }
    } catch (err) {
      this.results.gate2_agentic.passed = false;
      this.results.gate2_agentic.details.push(
        `❌ Agentic check failed: ${err.message}`,
      );
    }
  }
  ...
  gate3VersionConsistency() {
    ...
    try {
      ...
      this.results.gate3_version.passed = false;
      this.results.gate3_version.details.push(
  ...
  gate4TagUniqueness() {
    ...
    try {
      ...
      this.results.gate4_tag_unique.passed = false;
      this.results.gate4_tag_unique.details.push(
        `❌ Tag check failed: ${err.message}`,
      );
    }
  }
  ...
  gate6IntegrityFilter() {
    ...
    try {
      ...
      this.results.gate6_integrity.details.push(
