...
const os = require('os');
const { applyStandards, applyUkEnglish, generateFooter } = require('../../skills/apply-standards');
...
describe('apply-standards skill', () => {
  ...
  describe('generateFooter', () => {
    it('generates footer for block-plugin', () => {
      const frontmatter = { status: 'active', author: 'John Doe', version: '1.0.0' };
  ...
  describe('applyStandards', () => {
    ...
    it('skips files marked with meta:ignore', () => {
      ...
      fs.writeFileSync(filePath, '<!-- meta:ignore -->\n# Test\noptimized content');
    ...
    it('respects the repoType parameter', () => {
      const filePath = path.join(tempDir, 'test.md');
      fs.writeFileSync(filePath, '---\ntitle: Test\n---\n# Content');
      const result = applyStandards(filePath, { repoType: 'block-plugin', dryRun: true });
  ...
  describe('edge cases', () => {
    ...
    it('does not add duplicate footer blocks', () => {
      ...
      fs.writeFileSync(filePath, content);
      const result = applyStandards(filePath, { dryRun: true });
      const footerAdds = result.changes.filter(c => c.includes('footer')).length;
