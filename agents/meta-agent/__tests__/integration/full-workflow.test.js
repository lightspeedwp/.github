...
const { applyStandards } = require("../../skills/apply-standards.cjs");
const {
  generateBadges,
  injectBadges,
...
describe("Meta Agent v2.0 - Full Workflow Integration", () => {
  ...
  describe("Block Plugin Workflow", () => {
    ...
    it("applies standards to README", () => {
      ...
      const result = applyStandards(readmePath, {
    ...
    it("completes full workflow without errors", () => {
      ...
      const standardsResult = applyStandards(readmePath, {
        repoType,
        dryRun: false,
      });
  ...
  describe("Control-Plane Workflow", () => {
    ...
    it("applies standards to governance file", () => {
      ...
      const result = applyStandards(claudePath, {
  ...
  describe("Multi-file Workflow", () => {
    ...
    it("applies standards to multiple files", () => {
      const files = ["README.md", "DEVELOPMENT.md", "CHANGELOG.md"].map((f) =>
        path.join(tempDir, f),
      );
      const results = files.map((f) =>
        applyStandards(f, { repoType: "block-plugin", dryRun: false }),
      );
