...
- Use `@oai/artifact-tool` JS library for all spreadsheet authoring. It's installed in the bundled Codex primary runtime (`$CODEX_PRIMARY_RUNTIME`).
- Run builders with `$CODEX_PRIMARY_RUNTIME_NODE`. Do not use system, global, repo-local, or newly installed Node dependencies for workbook authoring.
- Work in a writable, conversation-specific OS tmp directory. In that working directory, create a `node_modules` symlink to `$CODEX_PRIMARY_RUNTIME_NODE_MODULES` so `import "@oai/artifact-tool"` resolves. Never modify the codex primary runtime directory or symlink from a synchronized workspace.
...
- For supporting analysis or data processing outside workbook authoring, use JS or spreadsheet formulas when sufficient. If Python is necessary, prefer $CODEX_PRIMARY_RUNTIME_PYTHON and its bundled libraries, save JSON/CSV intermediates, and have the JS builder create the workbook. Use existing system Python or user-provided libraries only when the bundled environment lacks a required capability. Do not install new packages. Keep auditable and user-editable calculations in the workbook as formulas.
- Use `update_plan` for complex spreadsheet work.

### Final Response

- Include a short user-visible summary and standalone Markdown link(s) only to final `.xlsx` artifact(s), one per line: `[Revenue Model - MNST.xlsx](sandbox:/absolute/path/to/revenue_model_mnst.xlsx)`.
