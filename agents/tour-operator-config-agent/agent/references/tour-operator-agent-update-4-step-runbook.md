...
## Prompt 4 — Run consistency checks, validate, report, and clean up temporary extracted files

Continue from the completed instruction and reference updates.

Now run consistency and validation checks.

Check for these issues:

- Does `AGENTS.md` mention every required reference file?
- Does every reference listed in `AGENTS.md` exist?
- Does the new content-model reference avoid contradicting the plugin-stack standard?
- Does the WordPress Tour Operator standard avoid treating extension content as core content?
- Does the content-structure mapping section still preserve its exact required output headings?
- Does the agent still preserve Discovery, Audit, Implementation, Reporting, and QA modes?
- Does Memory still stay short and factual?
- Are plugin facts clearly separated from commercial recommendations?
- Are Yoast and Gravity Forms still treated as supporting workflows?
- Are connected-tool limitations still stated clearly?
- Are production writes still treated as higher-risk?
- Are validation scripts or schema references still correct?

Run available validation scripts where possible. Prefer these checks when present:
...
If a validation script fails because of pre-existing unrelated gaps, do not hide that. Report:

- which script failed
- whether the failure is related to this update
- what should be addressed later

If scripts are unavailable, perform manual validation and say so.

If the source folder `tmp/tour-operator-agent-instructions` is no longer needed after all required merges and validation, remove it from the workspace. Do not remove any final merged files that were intentionally added to the main agent file tree.

When done, respond using exactly this structure:

## Existing Functionality Preserved

List the current behaviours preserved, including modes, workflows, plugin priorities, Gravity Forms, Yoast SEO, Memory, QA, and safety boundaries.

## Issues Found

List gaps or contradictions found in the current files.

Flag especially:

- missing content-model reference
- missing `AGENTS.md` pointer to the content-model reference
- contradiction between “Accommodation as optional strategy” and “Accommodation as built-in plugin CPT”
- extension content treated as core
- missing source register
- stale file references
- validation failures
- missing schemas or tests

## Changes Made

List each edited file and what changed.

## Files Added

List new files and why each was needed.

## Validation Completed

List scripts or manual checks completed.

## Remaining Follow-ups

List anything that still needs human review, builder-file attachment, file ID update, schema addition, or deeper source verification.

If creating a downloadable pack, place it in the output folder and include the link.

If only editing the workspace files, provide the file paths changed.

Keep the final response concise, practical, and honest about what could and could not be verified.
