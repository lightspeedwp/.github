...
RULES = {
    ...
    "references": re.compile(r"^[a-z0-9-]+(?:-reference|-standard|-policy|-checklist|-conventions|-runbook|-workflow)\.md$"),
    "tests": re.compile(r"^(?:test-plan|qa-checklist|regression)-[a-z0-9-]+\.md$|^[a-z0-9-]+-(?:checklist|tests|source|readme|snapshot)\.md$"),
    "scripts": re.compile(r"^validate-[a-z0-9-]+\.(?:sh|py)$|^run-[a-z0-9-]+\.sh$|^[a-z0-9-]+(?:-(?:script|helper|validator|automation))\.(?:sh|py|js)$"),
}

OPTIONAL_FOLDERS = {"templates", "examples", "memory"}
SKIP = {"README.md"}
