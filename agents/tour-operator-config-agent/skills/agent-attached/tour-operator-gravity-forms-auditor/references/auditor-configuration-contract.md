# Tour Operator Gravity Forms Auditor to Configuration Contract
...
Use this contract when `tour-operator-gravity-forms-auditor` identifies actionable Gravity Forms findings on a WordPress website running the Tour Operator plugin and relevant extension plugins that require approved remediation by the `tour-operator-gravity-forms-configuration` skill.

The auditor skill produces the handoff. The configuration skill accepts the handoff as evidence input, verifies the current site state, prepares a reversible change plan, seeks approval where required, applies only approved changes, and reports validation results.

This contract keeps the LightSpeed workflow clean: audit evidence to contract handoff to verified change plan to approval to implementation to validation to retest.
...
- Avoid test submissions, edits, activation/deactivation, settings changes, entry exposure, logging changes, payment checks, or secret exposure.
...
- Request approval for production-impacting, high-risk, payment, user-registration, privacy, retention, webhook, file-upload, notification overwrite, feed overwrite, cache, logging, analytics, or role/capability changes.
...
| `source_skill` | Yes | Usually `tour-operator-gravity-forms-auditor`. |
| `target_skill` | Yes | Must be `tour-operator-gravity-forms-configuration`. |
...
When receiving a handoff, `tour-operator-gravity-forms-configuration` must respond with:
...
| `recommended_retest` | Yes | Prompt or notes for `tour-operator-gravity-forms-auditor` retest. |
...
```json
{
  ...
  "handoff_id": "gf-audit-handoff-001",
  "handoff_title": "Fix unreliable contact form notifications",
  "source_audit": {
    "title": "Gravity Forms Fast Audit",
    "date": "2026-07-03",
    "source_skill": "tour-operator-gravity-forms-auditor"
  },
  "source_skill": "tour-operator-gravity-forms-auditor",
  "target_skill": "tour-operator-gravity-forms-configuration",
  ...
  "confirmed_evidence": [
    {
      "finding_id": "GF-001",
      "summary": "Admin notification exists but From/Reply-To alignment is not verified.",
      "confidence": "Medium"
    }
  ],
  "evidence_gaps": [
  ...
  "proposed_remediation": [
    ...
    "Confirm SMTP or transactional email configuration before relying on the form."
  ],
  ...
  "change_risk_notes": [
    "Incorrect notification settings can lose leads or expose enquiry details to the wrong recipient."
  ],
  "validation_steps": [
