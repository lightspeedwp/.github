...
Last reviewed: 2026-07-03. This register records the current source pass used for the expanded LightSpeed Tour Operator Gravity Forms Configuration skill. The attached user brief requested a fresh, systematic source review across product pages, templates, docs, add-on ecosystem pages, Zero Spam, and MCP action guidance. Where a supplied URL was not explicit in the pasted brief, the closest official/category source was used and marked with an implementation caveat.
...
- Prefer official Gravity Forms docs for behaviour, fields, add-ons, settings, REST API, security, privacy, accessibility, and troubleshooting.
- Use product pages and template pages for workflow patterns, not as proof that a specific field/add-on/feed exists on a live site.
- Use third-party add-on sources only after the site preflight confirms the plugin is installed, active, licensed where applicable, and supported.
- Mark Gravity Forms 3.0 behaviour as beta/version-specific unless the target site is confirmed to run a compatible 3.0 build and the user accepts beta risk.
- Re-run source review before changing payment, user registration, retention, upload, REST/API, or production publishing behaviour on a live site.

## 2026-07-03 improvement pass additions
...
- The Gravity Forms REST API v2 guide states that API v2 is part of core from Gravity Forms 2.4 and documents form, notification, feed, field, entry, and result endpoints. This supports API/MCP capability mapping but does not prove a specific WordPress connector exposes every endpoint.
- The Gravity Forms spam guidance recommends multiple anti-spam layers rather than relying on a single method. The skill should not promise complete spam elimination.
...
- Gravity Forms documents current preferred and minimum environment requirements and notes that requirements can change, so the skill records environment findings as time-sensitive and recommends current system-status checks before live work.
- Gravity Forms notification guidance separates entry creation from email delivery, so the skill now treats missing emails as a delivery investigation until notification config, SMTP, DNS, and logs are reviewed.
- Gravity Forms logging docs warn that logs can contain personal data in plain text, so the skill now requires temporary logging, redaction, and post-investigation cleanup notes.
- Gravity Forms upload guidance confirms the File Upload field is constrained by WordPress core upload and MIME validation, so the skill now blocks broad upload defaults and requires an approved extension/retention stance.
- Gravity Forms cache guidance identifies caching and script optimisation as common causes of dynamic form failures, so the skill now adds cache/CDN/script exclusion checks to troubleshooting and production embed planning.

## 2026-07-03 feed and entry-lifecycle improvement pass
...
| https://docs.gravityforms.com/stripe-add-on/ | official docs | Stripe add-on prerequisites, SSL, compatible form, feed setup, recurring payments | High | Last modified March 27, 2025. Payment work requires SSL, Stripe account, compatible form, and sandbox testing. |
...
- Feed processing depends on active feed state, add-on connection, conditional logic, field mappings, spam status, and background processing/log evidence.
- Payment add-ons can process only one payment feed per submission, so multiple payment feeds require tested conditional logic rather than assumption.
- User Registration feeds can create or update WordPress users; role assignment, password handling, activation, and Update User feed behaviour require explicit approval.
- Entry data, Save and Continue drafts, Partial Entries, uploaded files, IP addresses, and add-on destinations need a data-lifecycle stance before retention or export/erase changes.

## 2026-07-03 embed, consent, and localisation improvement pass
...
- Gravity Forms documents several supported embed paths. The skill should prefer the Gravity Forms block or Embed Form workflow on block-editor sites, but preserve legacy shortcodes when safer for an existing page.
- Gravity Forms shortcode parameters can affect AJAX, dynamic defaults, theme/style settings, and keyboard order; the skill now validates these before recommending production changes.
- Gravity Forms warns that embedding the same form more than once in a single page can cause submission, focus, payment, and CAPTCHA issues; the skill now treats duplicate same-form page embeds as approval/blocker work.
- Gravity Forms Consent field entries can record the consent wording presented at submission time and use state validation when wording changes; the skill now treats consent wording edits and translation changes as approval-required.

## 2026-07-03 conditional, dynamic, calculation, and webhook improvement pass
...
- Dynamic population can be configured through query strings, shortcode field values, the Gravity Forms block Field Values setting, or PHP hooks. Hook-based population requires developer routing unless the active toolchain safely supports code changes.
- Dynamic population uses PHP and may not work correctly on cached pages; the skill must check cache/CDN behaviour before using dynamic field values on live pages.
- Conditional logic can affect fields, pages, notifications, confirmations, and feeds; the skill now requires a dependency map and branch test coverage for live logic changes.
- Pricing fields can support quote generators, donations, and order-like forms, but payment-linked calculations remain high risk and require sandbox/feed validation.
- The Webhooks Add-On can send remote HTTP requests on form submission and supports URL, method, format, headers, body, selected fields, and conditions; the skill now prefers selected fields and redacted evidence rather than All Fields or exposed secrets.

## 2026-07-03 operations, inventory, and permissions improvement pass
...
- Gravity Forms exposes WordPress capabilities for creating/editing/deleting forms, viewing/editing/deleting/exporting entries, settings, updates, add-ons, System Status, logging, REST API settings, and uninstall; the skill now maps permissions by capability area rather than role name alone.
- Gravity Forms documentation warns that granting User Registration capabilities to non-admin roles can allow those roles to modify User Registration feeds and potentially escalate privileges; the skill therefore treats User Registration feed access as high/critical risk.
- The dashboard widget can show unread and total entries per form, but it is only a monitoring signal. It does not replace inventory, notification, feed, embed, or ownership checks.
- Gravity Forms scheduled events and background processors can affect feed processing, notifications, upgrades, retention cleanup, and add-on work. Delayed feeds/notifications should trigger cron/background-processing investigation before form-setting changes.

## 2026-07-03 frontend layout, styling, and theme improvement pass
...
- Gravity Forms themes can be applied globally or per embed via the block or shortcode. Global theme changes can affect many forms and should be preceded by inventory and regression checks.
- Orbital style controls cover common block-editor styling such as input size, border radius, colours, labels, descriptions, and button colours, but theme/plugin CSS can still affect the live page.
- CSS Ready Classes are legacy helpers; many layout classes are deprecated in favour of Form Editor columns, and Gravity Forms documentation warns that legacy markup support is planned for removal in Gravity Forms 4.0.
- Gravity Forms 2.5 introduced markup changes including `fieldset`/`legend` changes, removal of list wrappers for modern markup, and deferred scripts. Selector and CSS guidance must therefore be version and markup-mode aware.
- Frontend style changes must validate labels, required indicators, errors, focus states, mobile wrapping, confirmation states, CAPTCHA widgets, payment/total displays, and conditional branches.

## Ninth improvement batch: post creation and UGC workflows
...
- Gravity Forms Google Analytics Add-On can send submission and pagination events and supports Measurement Protocol, Google Analytics, Google Tag Manager, and manual configuration paths. The skill must detect whether the add-on is installed and connected before recommending add-on feed changes.
- Gravity Forms warns that duplicate scripts can double-count events when the add-on outputs tracking code while another plugin/theme/tag manager also adds it; the skill therefore checks existing tracking first.
- Gravity Forms dynamic population supports query strings, shortcode `field_values`, block Field Values, and hooks, but it is cache-sensitive and should avoid reserved WordPress terms.
- Gravity Forms confirms browser-side tracking services may record submit attempts before Gravity Forms validation and entry creation, so the skill separates attempted submissions from saved entries.
...
- Survey workflows require Survey Add-On detection before Survey fields, Survey results, or Survey merge tags are recommended.
- Survey results are internal dashboard views; summary results cannot currently be displayed on the front end or exported directly, so entry export may be needed for external analysis.
- Poll workflows require Polls Add-On detection before Poll fields, Poll block, Poll widget, poll shortcode, or public results are recommended.
- Quiz workflows require Quiz Add-On detection before Quiz fields, scoring, pass/fail or letter grades, quiz merge tags, or quiz results are recommended.
- Quiz scoring can be recalculated dynamically when correct answers or weighted scores change; hidden conditional quiz questions still count as incorrect.
- Assessment forms are not secure exam/proctoring systems by default. Route high-stakes testing, certification, employment, or compliance decisions to a separate approved workflow.

## Payment, donation and event payment batch reviewed 2026-07-03

| Source URL | Source type | Last reviewed | Main topic | Confidence | Notes |
|---|---|---:|---|---|---|
| https://docs.gravityforms.com/stripe-add-on/ | official docs | 2026-07-03 | Stripe Add-On prerequisites, one-time payments, recurring recurring payments, feeds, sales results and cancellation notes | High | Version-specific; requires installed add-on, SSL, connected Stripe account and test/live mode awareness. |
...
## tour-operator-gravity-forms-auditor boundary update prompt

- Source URL: user-provided pasted markdown in current conversation
- Source type: internal workflow prompt
- Last reviewed date: 2026-07-03
- Main topic: Boundary between `tour-operator-gravity-forms-configuration` and `tour-operator-gravity-forms-auditor`, including handoff contract requirements.
