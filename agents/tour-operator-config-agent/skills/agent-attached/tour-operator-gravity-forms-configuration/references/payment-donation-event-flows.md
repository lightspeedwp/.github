...
- Gravity Forms pricing fields can support quote generators, donation systems, simple paid forms and event registration style use cases, but this does not make Gravity Forms a replacement for a dedicated ecommerce, inventory, invoicing, or accounting workflow.
...
- Stripe supports one-time and recurring payments, but cancellation and lifecycle operations require clear operational ownership and are not a casual form-config edit.
...
- Refunds, failed payments, disputes, cancellations, recurring payment status changes, and accounting reconciliation are payment-operations work. Gravity Forms can expose entry/payment status signals, but the gateway/admin system remains the payment source of truth.
...
3. Installed payment add-ons and active gateway connection: Stripe, PayPal, Square, Mollie, Authorize.Net, Coupons, or other.
...
7. Notification behaviour: admin alert, user receipt, finance/admin copy, failure notice, recurring-payment/cancellation expectations.
...
- Treat recurring donations as recurring-payment architecture, not a minor form edit.

## Paid event defaults

For paid event forms:

- Confirm capacity, ticket inventory, cancellation/refund policy, attendee data needs, and whether a dedicated event platform should own the workflow instead.
...
- Recurring payment cancellation: require gateway/add-on evidence, entry/payment status, customer identity verification, and explicit approval.
...
- Dedicated ecommerce, inventory, fulfilment, accounting, recurring billing, or commercial platform architecture.
