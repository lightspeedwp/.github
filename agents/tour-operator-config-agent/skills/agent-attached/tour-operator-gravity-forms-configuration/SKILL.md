---
name: tour-operator-gravity-forms-configuration
description: configure, validate, troubleshoot, and document gravity forms setups for tour operator and travel websites, including enquiry forms, trip-planning forms, itinerary requests, quote request forms, brochure requests, destination or tour interest forms, conditional logic, notifications, confirmations, spam protection, accessibility, privacy, analytics, crm or webhook feeds, and post-change validation. use for tour operator gravity forms planning, approved implementation changes, readiness checks, troubleshooting, testing, and handoff notes. do not use for product catalogue, cart, fulfilment, recurring billing, or platform-specific ecommerce architecture.
---

# Tour Operator Gravity Forms Configuration
...
Treat the following as high risk unless clearly scoped and approved: payments, recurring billing, user registration, file uploads, public post creation, webhook/API feeds, entry export/deletion, retention changes, consent wording, production embeds, notification overwrites, role/capability changes, analytics/tracking changes, cache/script exclusions, live multilingual changes, broad frontend styling changes, and any change that may lose leads, expose personal data, misroute submissions, or trigger external systems.

## When to use this skill

Use this skill for Gravity Forms implementation, lightweight fast audit snapshots before configuration, lightweight preflight/readiness checks before configuration, new form planning, existing form remediation, notification/confirmation setup, feed/add-on setup, spam/security configuration, accessibility-aware configuration fixes, tour operator enquiry forms, trip-planning forms, itinerary requests, brochure requests, destination interest forms, travel quote requests, payment/donation/event payment flows, UGC/post-creation implementation, analytics/tracking configuration for forms, surveys/polls/quizzes where configuration is required, approved MCP change workflows, post-change validation, troubleshooting, remediation handoffs, and LightSpeed handoff notes.

Use this skill to implement or plan remediation from a `tour-operator-gravity-forms-auditor` handoff after verifying current site state. Do not use it as the primary owner for full audit reports, findings registers, scorecards, client-safe audit summaries, or retainer health-check reports.

## When not to use this skill

Do not use it for general WordPress configuration unrelated to Gravity Forms, custom plugin development, legal privacy-policy drafting, whole-site accessibility or performance audits, visual Figma design work, page copywriting beyond form microcopy, launch QA beyond Gravity Forms-specific checks, or payment, recurring billing, or commercial pricing architecture that is not explicitly scoped as a Gravity Forms form flow.

Do not use this skill for full Gravity Forms audit reports, formal findings registers, readiness scorecards, client-safe audit summaries, retainer audit reports, or retest reports. Use `tour-operator-gravity-forms-auditor` for those, then return to this skill for approved remediation planning and implementation.

## Auditor/configuration boundary

Use `tour-operator-gravity-forms-auditor` when the requested outcome is a formal read-only audit, scorecard, inspection report, comparison report, findings register, client-safe audit summary, retainer health check, or separate retest report.
...
When receiving a `tour-operator-gravity-forms-auditor` handoff, load `references/auditor-configuration-contract.md`.
...
- If the user needs scoring, a formal findings register, client-safe reporting, retainer audit, or broad multi-form review, route to `tour-operator-gravity-forms-auditor`.
...
Never assume licence tier, add-on availability, gateway configuration, or third-party plugin support. Detect installed and active add-ons before recommending feeds or fields for User Registration, Stripe, PayPal, Square, Coupons, Partial Entries, Save and Continue, Webhooks, Mailchimp, Zapier, Slack, Dropbox, Help Scout, Salesforce, Cloudflare Turnstile, Gravity Perks, GravityKit, or Zero Spam.
...
Payments, recurring billing, travel deposits, account creation, role assignment, activation emails, and password flows require explicit approval and evidence that the relevant official add-on/gateway is installed, licensed, configured, and appropriate. Prefer draft plans and test-mode validation before live changes.

## Tour-operator-specific routing

For tour operators, prefer enquiry-first and consultant-assisted trip-planning forms over full booking automation unless operations, availability, deposits, cancellation, and handoff rules are approved. Prioritise itinerary requests, brochure requests, destination or tour interest forms, quote requests, travel dates, group size, accommodation preferences, traveller interests, budget range, consent, and lead-routing fields. Use multi-page forms and conditional logic to reduce friction, support partial entries only when the add-on is confirmed and approved, and avoid collecting sensitive travel documents unless secure handling is confirmed. Load `profiles/tour-operator-configuration-agent.md` first for agent defaults.
...
Choose one primary output and keep it evidence-labelled. If the requested output is a full audit report, findings register, readiness scorecard, client-safe audit summary, retainer health check, or separate retest report, route to `tour-operator-gravity-forms-auditor` instead of producing it here.
...
- Donation, paid event, standalone payment, receipt, refund, cancellation and payment-test workflows: `references/payment-donation-event-flows.md`
...
- Agent-specific defaults: `profiles/tour-operator-configuration-agent.md` first, with `profiles/wordpress-configuration-agent.md` only for general WordPress form fallback
